...
struct OdTrVisLightDef
{
  ...
  enum Overrider
  {
    ...
    kOverrideColor         = (1 << 2),
    kOverrideTextrue       = (1 << 3),
    kOverrideAttenuation   = (1 << 4),
    kOverrideAngles        = (1 << 5),
    kOverrideSpecualFactor = (1 << 6),
    kOverrideShadow        = (1 << 7),
  ...
  OdTrVisLightDef &apply(OdUInt32 nMod, const OdTrVisLightDef &l)
  {
    if (nMod != 0xFFFFFFFF)
    {
      ...
      if (GETBIT(nMod, kOverrideOrientation))
        ...
        m_upVector = l.m_upVector;
      if (GETBIT(nMod, kOverrideColor))
        m_color = l.m_color;
      if (GETBIT(nMod, kOverrideTextrue))
  ...
  OdUInt32 diff(const OdTrVisLightDef &dl2, bool bFinFirstDiff = false) const
  {
    ...
    if (!m_position.isEqualTo(dl2.m_position) || !m_direction.isEqualTo(dl2.m_direction) ||
        !m_upVector.isEqualTo(dl2.m_upVector))
    { if (bFinFirstDiff)
        return 0xFFFFFFFF;
    ...
    if (m_texture != dl2.m_texture)
    { if (bFinFirstDiff)
        return 0xFFFFFFFF;
      SETBIT_1(axxum, kOverrideTextrue);
    }
...
inline void OdTrVisLightDef::load(OdGsFiler *pFiler) {
  ...
  pFiler->rdVector3d(m_upVector);
  for (int nColorElem = 0; nColorElem < 4; nColorElem++)
    m_color[nColorElem]   = pFiler->rdFloat();
  m_texture               = (OdTrVisTextureId)pFiler->rdUInt64();
