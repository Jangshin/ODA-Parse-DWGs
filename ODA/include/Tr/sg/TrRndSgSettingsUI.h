...
class OdTrRndSgSceneGraphSettingsUI : public ExtBar::Callback
{
  ...
  protected:
    enum UIElements
    {
      ...
      kUIShareUnionsCheckBox,      kUIShareUnionsText,
      ...
      kUIMinUnionDrawRangeBox,     kUIMinUnionDrawText,
  ...
  protected:
    ...
    OdUInt8                    m_prcMinWeight;         // Minimal balancing weight for weighted balancing mode
    OdUInt8                    m_prcMinDepth;          // Minimal balancing tree level (in percents of entire tree depth)
    OdUInt8                    m_prcMergeDepth;        // Merging inclusion childs depth (in percents of entire tree depth)
    OdUInt8                    m_prcMinPercDraw;       // Minimal union volume before explode (in percents of initial volume)
  ...
  public:
    ...
    void reset()
    {
      m_ngsFlags = kColorDataExt | kLineStripsExplode | kFlattenTtfTexts | kExplodeBlockRefs | kAllowNestedSceneGraphs | kEmbedOrdering | kPurgeGrouppedRenders | kShareUnions;
    ...
    void setUnionsSharing(bool bEnable) { SETBIT(m_ngsFlags, kShareUnions, bEnable); }
    bool shareUnions() const { return GETBIT(m_ngsFlags, kShareUnions); }
    ...
    void setWeightedBalancingLimit(OdUInt32 weightLimit) { m_prcMinWeight = (OdUInt8)weightLimit; }
    OdUInt32 weightedBalancingLimit() const { return m_prcMinWeight; }

    void setBalancingLevelLimit(OdUInt32 minDepthLimit) { m_prcMinDepth = (OdUInt8)minDepthLimit; }
    OdUInt32 balancingLevelLimit() const { return m_prcMinDepth; }

    void setBalancingMergeDepth(OdUInt32 mergeDepth) { m_prcMergeDepth = (OdUInt8)mergeDepth; }
    OdUInt32 balancingMergeDepth() const { return m_prcMergeDepth; }

    void setMinimalUnionPercentsDraw(OdUInt32 minDrawPerc) { m_prcMinPercDraw = (OdUInt8)minDrawPerc; }
    OdUInt32 minimalUnionPercentsDraw() const { return m_prcMinPercDraw; }
    ...
    static void copyGlobalSettings(TypeTo &to, const TypeFrom &from)
    {
      ...
      to.setUnionsSharing(from.shareUnions());
      ...
      to.setMinimalUnionPercentsDraw(from.minimalUnionPercentsDraw());
    ...
    { // Initialize Global Settings
      ...
      // Share Unions
      pExtBar->createCheckBox(kUIShareUnionsCheckBox, pGroup, ExtBar::k8d8, shareUnions())->setText(OD_T("Share Unions"));
      // Scene Tree Type
      ...
      // Minimal Union Percents
      pExtBar->createText(kUIMinUnionDrawText, pGroup, ExtBar::k6d8)->setText(OD_T("Minimal Union to Draw (%):"));
      pExtBar->createRangeBox(kUIMinUnionDrawRangeBox, pGroup, ExtBar::k2d8)->setUInt(minimalUnionPercentsDraw(), 0, 100);
    ...
    { if (!nReason) return;
      ...
      setUnionsSharing(ExtBar::CheckElement::cast(pExtBar->getElement(kUIShareUnionsCheckBox))->isChecked());
      ...
      setWeightedBalancingLimit(ExtBar::RangeElement::cast(pExtBar->getElement(kUIWeightLimitRangeBox))->getPosUInt());
      setBalancingLevelLimit(ExtBar::RangeElement::cast(pExtBar->getElement(kUIBalancingLimitRangeBox))->getPosUInt());
      setBalancingMergeDepth(ExtBar::RangeElement::cast(pExtBar->getElement(kUIBalancingDepthRangeBox))->getPosUInt());
      setMinimalUnionPercentsDraw(ExtBar::RangeElement::cast(pExtBar->getElement(kUIMinUnionDrawRangeBox))->getPosUInt());
      ...
      setThreadsMaximum(ExtBar::RangeElement::cast(pExtBar->getElement(kUINumThreadsRangeBox))->getPosUInt());
