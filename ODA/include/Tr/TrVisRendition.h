...
class OdTrVisRendition : public OdRxObject
{
  public:
    ...
    { //kNoData = 0, // No data type entrance
      ...
      kRefPlaneData,    kRefPlanePropsData,
    ...
    enum DisplayIdCodes
    {
      ...
      kDisplayCodeTfMarkers   = 0x09, // Setup transformation markers
      kDisplayCodeDrawOrder   = 0x0A, // Provide drawing order for upcoming geometry
      kDisplayCodeDepthIval   = 0x0B, // Modify drawing order interval
      kDisplayCodeExploded    = 0x0C, // Initiates exploded union geometry rendering
      kDisplayCodeNonExploded = 0x0D, // Stops exploded union geometry rendering
      kDisplayCodeSelStyle    = 0x0E, // Setup selection style
      kDisplayCodeSelectionID = 0x0F, // Entity id for selection
  ...
  public:
    ...
    struct ClientSettings
    {
      ...
      enum Flags
      {
        ...
        kShadeByFragment = (1 << 9), // Fragment lighting mostly not influence onto final performance, so we can
                                     // force per-fragment lighting for visual styles, which disable it.
        // Data precision
      ...
      bool isShadeForceByFragment() const { return GETBIT(m_uFlags, kShadeByFragment); }
      void setShadeForceByFragment(bool bSet) { SETBIT(m_uFlags, kShadeByFragment, bSet); }
  ...
  public:
    ...
    */
    ...
    // Extract local rendition-specific host service.
    ...
    // Called when exist metafile extents updated (affected by transformation state branches).
    ...
    // State branches (highlighting, hide, transform)
    ...
    // Called to attach additional data to range of state branch markers.
    virtual void onStateBranchMarkersRangeDataAdded(OdTrVisStateBranchId stateBranchId, const OdTrVisStateBranchMarkersRange &range,
                                                    const OdTrVisPropertyDef &propDef) = 0;
    // Called to detach additional data from state branch markers range.
    ...
    // Called when texture flags modified.
    ...
    // Called when new material reflection plane added.
    virtual void onMaterialRefPlaneAdded(OdTrVisRefPlaneId refPlaneId, const OdTrVisRefPlaneDef &pDef) = 0;
    // Called when exist material reflection plane deleted.
...
class OdTrVisRenditionDef : public OdTrVisRendition
{
  ...
  public:
    ...
    // Extract local rendition-specific host service.
    virtual OdTrRndLocalRenditionHost *localRenditionHost() { return nullptr; }
...
class OdTrVisRenditionRedir : public OdTrVisRendition
{
  ...
  public:
    ...
    // Renders viewport to selection texture. Return top object id inside rect(aPtDc[0], aPtDc[1]) in viewport.
    virtual void getSelectedObject(OdTrVisViewportId id, const OdGePoint2d* aPtDc, int numPoints, OdArray<OdTrVisId> &outIDs, OdTrVisGsMarker *outMarker = NULL) ODRX_OVERRIDE
    {
      if (hasInformationalRedirection())
        getInformationalRedirection()->getSelectedObject(id, aPtDc, numPoints, outIDs, outMarker);
    }
    ...
    {
      if (hasInformationalRedirection())
        return getInformationalRedirection()->localRenditionHost();
      return nullptr;
    }
    ...
    {
      if (hasRedirection())
        getRedirection()->onViewportBackgroundChanged(viewportId, pDef);
    }
    // Called when viewport fog changed.
    virtual void onViewportFogChanged(OdTrVisViewportId viewportId, const OdTrVisFogDef &pDef)
    {
    ...
    {
      if (hasRedirection())
        getRedirection()->onMetafileExtentsUpdated(metafileId, extents);
    }
    // Called when exist metafile visibility changed.
    virtual void onMetafileVisibilityChanged(OdTrVisMetafileId metafileId, bool bVisibility)
    {
    ...
    {
      if (hasRedirection())
        getRedirection()->onStateBranchMarkersRangeDataAdded(stateBranchId, range, propDef);
    }
    // Called to detach additional data from state branch markers range.
    virtual void onStateBranchMarkersRangeDataRemoved(OdTrVisStateBranchId stateBranchId, const OdTrVisStateBranchMarkersRange &range)
    {
      if (hasRedirection())
        getRedirection()->onStateBranchMarkersRangeDataRemoved(stateBranchId, range);
    }
    ...
    {
      if (hasRedirection())
        getRedirection()->onTextureFlagsModified(textureId, newFlags);
    }
    ...
    {
      if (hasRedirection())
        getRedirection()->onMaterialRefPlaneAdded(refPlaneId, pDef);
    }
    // Called when exist material reflection plane deleted.
    virtual void onMaterialRefPlaneDeleted(OdTrVisRefPlaneId refPlaneId)
    {
      if (hasRedirection())
        getRedirection()->onMaterialRefPlaneDeleted(refPlaneId);
    }
