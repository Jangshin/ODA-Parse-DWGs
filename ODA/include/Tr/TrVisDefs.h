...
// Uncomment to disable all prefetchers, which influence onto stability of MT-issues reproduction.
//#define OD_TRVIS_SLOW_DEBUG
...
namespace{
static inline bool isValidMarker(OdTrVisGsMarker marker){
...
// There are several enumerations where order of data this entries should be compatible, so define following enumeration globally.
enum class OdTrVisNormalType : int
{
  kPrimary, kSecondary,
  kBumpMappingT, kBumpMappingB,
  kNormalMapT, kNormalMapB, kNumTypes
};
enum class OdTrVisTextureType : int
{
  kDiffuse, kOpacity, kBumpMap, kSpecular, kRefraction,
  kEmission, kNormalMap, kRoughness, kCutouts, kReflection, kNumTypes
};
...
struct OdTrVisSectionPlane : public OdTrVisSubMat
{
  ...
  void smartTransformBy(const OdGeMatrix3d &xfm) {
    OdGePoint3d pos = xfm * position(); OdGeVector3d norm = xfm * normal();
    double dist = norm.dotProduct(pos.asVector());
    pos = OdGePoint3d::kOrigin + norm * dist;
    set(pos, norm);
  }

  template<typename ValType> bool isPtOnPlane(ValType x, ValType y, ValType z, float tol = 1.e-8f) const
  { return Od_abs((x - entry[0][0]) * entry[1][0] + (y - entry[0][1]) * entry[1][1] + (z - entry[0][2]) * entry[1][2]) < tol; }
  template<typename VecType> bool isPtOnPlane(const VecType &pt, float tol = 1.e-8f) const
  { return isPtOnPlane(pt.x, pt.y, pt.z, tol); }
...
inline bool OdTrVisSectionPlane::operator ==(const OdTrVisSectionPlane &sp) const
{
  return OdTrVisFPEqual(entry[1][0], sp.entry[1][0]) && OdTrVisFPEqual(entry[1][1], sp.entry[1][1]) &&
         OdTrVisFPEqual(entry[1][2], sp.entry[1][2]) && isPtOnPlane(sp.entry[0][0], sp.entry[0][1], sp.entry[0][2]);
}
inline bool OdTrVisSectionPlane::operator !=(const OdTrVisSectionPlane &sp) const
{
  return !OdTrVisFPEqual(entry[1][0], sp.entry[1][0]) || !OdTrVisFPEqual(entry[1][1], sp.entry[1][1]) ||
