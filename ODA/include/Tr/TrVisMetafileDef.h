...
struct OdTrVisMetafileContainer : public OdTrVisFlatMetafileContainer
{
  ...
  OdUInt16           m_rtFlags;     // Ray-trace related flags
  // Note: for 64bit configuration we has Int32 padding here, which can be used without container size grow.
  //       Or we can increase flags container size.
  OdIntPtr           m_revData;     // Allocation address
  OdTrRndSgExpand   *m_pSgData;     // Runtime scene graph rendering data

  OdTrVisMetafileContainer() : m_mfFlags(0), m_rtFlags(0), m_revData(0), m_pSgData(NULL) {}

  bool hasRevData() const { return m_revData != 0; }
  OdIntPtr getRevData() const { return m_revData; }
  void setRevData(OdIntPtr revData) { m_revData = revData; }
  ...
  { OdTrVisMetafileContainer *pClone = static_cast<OdTrVisMetafileContainer*>(OdTrVisFlatMetafileContainer::clone(pGaps));
...
#else
    static void addRef(OdTrVisMetafileContainer *pObj) { if (pObj) { OdTrVisAtomics::interlockedIncrement32(pObj->m_nRefs); } }
    static void release(OdTrVisMetafileContainer *pObj) { if (pObj && !OdTrVisAtomics::interlockedDecrement32(pObj->m_nRefs)) pObj->selfRelease(); }
#endif
};
