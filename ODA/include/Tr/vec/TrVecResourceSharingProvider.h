...
class OdTrVecRefPlanesContainer;
class OdTrVisMetafilesPool;
class OdTrVisMtContext;
                                     ...
                                     , public OdDrawOrderBaseDeviceMCCallback
{
  ...
  protected:
    struct DeviceEntry
    {
      enum Flags
      {
        kConnected = (1 << 0), // Device connected to sharing provider
        kDeleted   = (1 << 1)  // Device physically deleted and haven't ability to reconnect
      };
  ...
  public:
    ...
    // Share metafiles group with secondary device (base call, will be redirected to secondary override)
    virtual bool shareGroup(OdTrVecDevice *pRequestedFor, OdDrawOrderMetafileBase &bitSet,
