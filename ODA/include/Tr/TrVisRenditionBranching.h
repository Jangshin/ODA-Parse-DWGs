...
class OdTrVisRenditionBranching : public OdTrVisRendition
{
  ...
  public:
    ...
    {
      if (hasRenditionA())
        getRenditionA()->getSelectedObject(id, aPtDc, numPoints, outIDs, outMarker);
      if (hasRenditionB())
        getRenditionB()->getSelectedObject(id, aPtDc, numPoints, outIDs, outMarker);
    }
    ...
    { // @@@TODO: probably we need branching of sharing providers?
      if (hasRenditionA())
        return getRenditionA()->localRenditionHost();
      if (hasRenditionB())
        return getRenditionB()->localRenditionHost();
      return nullptr;
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onViewportBackgroundChanged(viewportId, pDef);
      if (hasRenditionB())
        getRenditionB()->onViewportBackgroundChanged(viewportId, pDef);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onMetafileExtentsUpdated(metafileId, extents);
      if (hasRenditionB())
        getRenditionB()->onMetafileExtentsUpdated(metafileId, extents);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchMarkersRangeDataAdded(stateBranchId, range, propDef);
      if (hasRenditionB())
        getRenditionB()->onStateBranchMarkersRangeDataAdded(stateBranchId, range, propDef);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onStateBranchMarkersRangeDataRemoved(stateBranchId, range);
      if (hasRenditionB())
        getRenditionB()->onStateBranchMarkersRangeDataRemoved(stateBranchId, range);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onTextureFlagsModified(textureId, newFlags);
      if (hasRenditionB())
        getRenditionB()->onTextureFlagsModified(textureId, newFlags);
    }
    ...
    {
      if (hasRenditionA())
        getRenditionA()->onMaterialRefPlaneAdded(refPlaneId, pDef);
      if (hasRenditionB())
        getRenditionB()->onMaterialRefPlaneAdded(refPlaneId, pDef);
    }
    // Called when exist material reflection plane deleted.
    virtual void onMaterialRefPlaneDeleted(OdTrVisRefPlaneId refPlaneId)
    {
      if (hasRenditionA())
        getRenditionA()->onMaterialRefPlaneDeleted(refPlaneId);
      if (hasRenditionB())
        getRenditionB()->onMaterialRefPlaneDeleted(refPlaneId);
    }
