...
// State branch childs iterator
typedef const void *OdTrVisStateChildIt;
// State branch markers pointer
typedef const void *OdTrVisStateMarkerPtr;
// State branch markers iterator
struct OdTrVisStateMarkerIt
{
  OdTrVisStateMarkerPtr m_pData[4];
  OdTrVisStateMarkerIt(OdTrVisStateMarkerPtr pPtr = nullptr) { m_pData[0] = pPtr; }
  operator OdTrVisStateMarkerPtr() const { return *m_pData; }
