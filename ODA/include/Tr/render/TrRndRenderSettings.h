...
class OdTrRndRenderSettings
{
  ...
  public:
    enum DrawFlags
    {
      ...
      kDrawShadows,                     // Draw shadows
      kDrawFog,                         // Draw fog effect
      kDrawAntiAliasing,                // Draw anti-aliasing effect
      ...
      kFillFacesTexture,                // Use texture to fill faces
      ...
      kFlatLighting,                    // Ignore light sources during Phong lighting calculation
      kSpecularHighlighting,            // Enable specular highlighting
      kSpecularOverride,                // Override specular highlighting for default materials
      kShadingMaterial,                 // Use shading material instead of realistic material
      ...
      kDrawTriangulation,               // Draw faces triangulation
      // FX-related
      ...
      kDisableVSOverride,               // Disable Visual Style overrides for FX rendering
      ...
      kOverlappingDecision,             // Setup to draw on top of exist geometry with same coordinates
      kSkipImages,                      // Skip raster image textures application
...
#define OdTrRndRS_SetFor2dWireframe                { kDraw3dFacets, kDraw2dFacets, kDrawRasterImages, kDraw2dFacetEdges, kDrawIsolines, \
                                                     kDrawOrder, kDrawBackground, kDrawLineweights, kDrawAntiAliasing, \
                                                     kNullFlag }
#define OdTrRndRS_SetFor3dWireframe                { kDraw2dFacets, kDraw2dFacetEdges, kDrawRasterImages, kDrawIsolines, \
                                                     kDrawBackground, kDrawLineweights, kDrawAntiAliasing, \
                                                     kNullFlag }
...
#define OdTrRndRS_SetForFlatShaded                 { kDraw3dFacets, kDraw2dFacets, kDrawRasterImages, \
                                                     kDrawBackground, kDrawLineweights, kDrawLighting, kDrawShadows, kDrawFog, kDrawAntiAliasing, \
...
#define OdTrRndRS_FX                               { kDrawFX, kNullFlag }
    ...
    {
      // Faces
      kPropFacesSpecularPower = 0,      // Material specular power coefficient override
    ...
    enum IntegerProps
    {
      ...
      // Faces
      kPropTextureChannels,             // Set of inclusive flags to enable separate material texture channels
      // Edges
    ...
    enum MultiPassType
    {
      ...
      kMultiPassForSections,      // Multi-pass rendering for hardware sections
    ...
    enum StencilFlags
    {
      ...
      kStencilDFailBit = (1 << 12), kStencilDPassBit = (1 << 13), kStencilDepthMask = kStencilDFailBit | kStencilDPassBit
    };
    ...
    // kPropTextureChannels property settings
    enum TextureChannelFlags
    {
      kTextureDiffuseChannel   = (1 << unsigned(OdTrVisTextureType::kDiffuse)),   kTextureOpacityChannel    = (1 << unsigned(OdTrVisTextureType::kOpacity)),
      kTextureBumpMapChannel   = (1 << unsigned(OdTrVisTextureType::kBumpMap)),   kTextureReflectionChannel = (1 << unsigned(OdTrVisTextureType::kReflection)),
      kTextureSpecularChannel  = (1 << unsigned(OdTrVisTextureType::kSpecular)),  kTextureRefractionChannel = (1 << unsigned(OdTrVisTextureType::kRefraction)),
      kTextureEmissionChannel  = (1 << unsigned(OdTrVisTextureType::kEmission)),  kTextureNormalMapChannel  = (1 << unsigned(OdTrVisTextureType::kNormalMap)),
      kTextureRoughnessChannel = (1 << unsigned(OdTrVisTextureType::kRoughness)), kTextureCutoutsChannel    = (1 << unsigned(OdTrVisTextureType::kCutouts)),
      kTextureAllChannels = kTextureDiffuseChannel   | kTextureOpacityChannel    | kTextureBumpMapChannel  | kTextureReflectionChannel |
                            kTextureSpecularChannel  | kTextureRefractionChannel | kTextureEmissionChannel | kTextureNormalMapChannel  |
                            kTextureRoughnessChannel | kTextureCutoutsChannel
    };
  ...
  protected:
    ...
    OdUInt16 m_rsPassId;                  // User-defined pass Id
  ...
  public:
    ...
    OdTrRndRenderSettingsPtr searchPassId(OdUInt16 passId, OdTrRndRenderSettingsPtr pSelfPtr = OdTrRndRenderSettingsPtr(),
                                          OdUInt16 passMask = 0xFFFF, bool bSearchNested = true) const;
    ...
    {
      if (m_pNextPass[passType].get() != pNextPass)
        setNextPass(passType, OdTrRndRenderSettingsPtr(pNextPass), rsReact);
    }
    ...
    {
      if (m_pNextPass[passType].get() != pNextPass.get())
      {
    ...
    {
      return !drawFlag(kDrawOrder) && ((drawFlag(kDraw3dFacets) && (drawFlag(kDraw3dFacetEdges) || drawFlag(kDrawIsolines) || drawFlag(kDrawTriangulation))) ||
                                                                    drawFlag(kDrawIntersections) || drawFlag(kDrawSilhouettes));
    }
    ...
    {
      return drawFlag(kGoochLighting) || drawFlag(kFlatLighting);
    }
