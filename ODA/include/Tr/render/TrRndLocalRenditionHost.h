...
class OdTrRndLocalRenditionHost : protected OdTrRndSystemExternalArraysManager
{
  public:
    ...
    // Check does client rendition require metafile data unpurge, or support alternative.
    virtual bool unpurgeRequirement() const { return true; }
    ...
    // Some renderers require own unique bindings for data arrays. In this case we can add special array for per-rendition bindings.
    virtual bool requireUniqueBindings(const OdTrRndLocalRenditionHost * /*pSecondary*/) const { return false; }
    virtual void toggleUniqueBindings(OdTrVisMetafileContainer * /*pMfStream*/, const OdTrRndLocalRenditionHost * /*pSecondary*/) const { }
