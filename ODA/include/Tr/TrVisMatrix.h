...
struct OdTrVisMatrix4x4Impl
{
  ...
  { 
    return OdTrVisMiniVec4d<VectorDataType>(
      static_cast<VectorDataType>(entry[0][0]), 
      static_cast<VectorDataType>(entry[1][0]), 
      static_cast<VectorDataType>(entry[2][0]), 
      static_cast<VectorDataType>(entry[3][0])); 
  }
  template <typename VectorDataType = DataType>
  OdTrVisMiniVec4d<VectorDataType> getRotationYAxis4() const
  { 
    return OdTrVisMiniVec4d<VectorDataType>(
      static_cast<VectorDataType>(entry[0][1]), 
      static_cast<VectorDataType>(entry[1][1]), 
      static_cast<VectorDataType>(entry[2][1]), 
      static_cast<VectorDataType>(entry[3][1])); 
  }
  template <typename VectorDataType = DataType>
  OdTrVisMiniVec4d<VectorDataType> getRotationZAxis4() const
  { 
    return OdTrVisMiniVec4d<VectorDataType>(
      static_cast<VectorDataType>(entry[0][2]), 
      static_cast<VectorDataType>(entry[1][2]), 
      static_cast<VectorDataType>(entry[2][2]), 
      static_cast<VectorDataType>(entry[3][2])); 
  }
  template <typename VectorDataType = DataType>
  OdTrVisMiniVec4d<VectorDataType> getTranslation4() const
  { 
    return OdTrVisMiniVec4d<VectorDataType>(
      static_cast<VectorDataType>(entry[0][3]), 
      static_cast<VectorDataType>(entry[1][3]), 
      static_cast<VectorDataType>(entry[2][3]), 
      static_cast<VectorDataType>(entry[3][3])); 
  }
  ...
  { return vec.set(entry[0][0] * vec.x + entry[1][0] * vec.y + entry[2][0] * vec.z + entry[3][0] * vec.w,
                   entry[0][1] * vec.x + entry[1][1] * vec.y + entry[2][1] * vec.z + entry[3][1] * vec.w,
                   entry[0][2] * vec.x + entry[1][2] * vec.y + entry[2][2] * vec.z + entry[3][2] * vec.w,
                   entry[0][3] * vec.x + entry[1][3] * vec.y + entry[2][3] * vec.z + entry[3][2] * vec.w); }
