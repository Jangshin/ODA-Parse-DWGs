...
class OdTrVisMetafileParser
{
  public:
    ...
    struct VarState
    {
      ...
      const float *asFVec(float *out, OdUInt32 count = 1) const;
      const double *asDVec(double *out, OdUInt32 count = 1) const;
    };
  ...
  public:
    ...
    OdUInt32 streamPos() const { return OdUInt32(m_curState.m_caret.m_pStreamCur - m_initState.m_pStreamStart); }
...
template <class T> T align(const void* ptr)
{ 
  T res; 
...
inline const float*        OdTrVisMetafileParser::VarState::asFVec()       const { return align<const float*>(&m_pVar); } // address of pointer to mem copy pointer, not value
inline const double*       OdTrVisMetafileParser::VarState::asDVec()       const { return align<const double*>(&m_pVar); } // address of pointer to mem copy pointer, not value
...
inline const double *OdTrVisMetafileParser::VarState::asDVec(double *out, OdUInt32 count) const
{
  memcpy(out, m_pVar, sizeof(double) * count);
  return out;
}
