...
public:
  ...
  virtual bool getRscFontFractionInterval(OdDgFontTable* pFontTable, const OdString& strRscFontName, OdChar& chFirst, OdChar& chLast) const;
  virtual bool getRscFontFractionInterval(OdDgFontTable* pFontTable, OdUInt32 uFontNumber, OdChar& chFirst, OdChar& chLast) const;
  virtual void rscFontNotFound( OdUInt32 uFontIndex );
  virtual bool isFilledRscFont(OdDgFontTable* pFontTable, const OdString& strRscFontName);
};
...
/** \details
    This class defines the interface for load of dwg line styles from *.lin file to dwg database.
    Default implementation of the protocol extension is on TG_DwgDb module.
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgLoadLineStyleFromLinFilePE : public OdRxObject
{
public:
  ODRX_DECLARE_MEMBERS(OdDgLoadLineStyleFromLinFilePE);

  virtual bool loadLineStyleFromLinFile(OdDgDatabase* pDgnDb, const OdString& strFileName) = 0;
};

/** \details
  This template class is a specialization of the OdSmartPtr class
  for OdDgLoadLineStyleFromLinFilePE object pointers.
*/
typedef OdSmartPtr<OdDgLoadLineStyleFromLinFilePE> OdDgLoadLineStyleFromLinFilePEPtr;
