...
public:
  ...
  */
  ...
  /** \details
    This function is called by application to register command context object.
    Callback functions than can retrieve command context object for user interaction.

    \param db [in] database command context object is related to.
    \param dbctx [in] command context object. When NULL context will be unregistered.

    \sa
    registredCommandContext
  */
  static OdResult registerCommandContext(OdDbDatabase* db, OdDbCommandContext* dbctx);

  /** \details
    This function retrieves command context object for user interactions.
  ...
  */
  static OdDbCommandContext* registredCommandContext(OdDbDatabase* db);

  /** \details
      Returns host application services (by default database()->appServices()).
      Override to return correct value when there is no database yet.
  */
  virtual OdDbHostAppServices* appServices() const;
};
