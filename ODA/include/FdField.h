...
public:
  ...
  */
  ...
  /** \details
    Converts a value from one units of measurement to another e.g. inches to millimeters.

    \param field [in] Input value.
    \param field [in] Source units name.
    \param field [in] Destination units name.
    \param field [out] Conversion result.

    \returns eOk if the input value was successfully converted,
    eInvalidInput if the source units or destination units are either not recognized or are incompatible
  */
...
/** \details
  ...
  \param countTextFields  [in]  Defines if top level "TEXT" fields are counted. UPDATEFIELD command 
                                does not count them.
