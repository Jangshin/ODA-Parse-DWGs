...
public:
  ...
  /* The output stream is binary data stored inside entity. Without any parsing\resaving.*/
  virtual OdResult brepOutNative(const OdDgCellHeader3d *pCell, OdStreamBuf &externalBinData, OdStreamBuf &internalBinData) const;
  ...
  void fillSmartSolid(OdDgCellHeader3d& p3d, OdStreamBuf &binData, bool bBrepIsAcis, double dScale = 0, bool bCreateSurface = false) const;
  ...
  virtual bool isBrepCacheSynchronized(const OdDgCellHeader3d& p3d, const OdGeMatrix3d& mx, OdDgViewPtr& pView, OdStreamBufPtr &binData, OdGiWorldDraw* pWd = NULL) const;

  // return extents of brep element from internal stream
  virtual OdResult getBrepDataExtents(const OdDgCellHeader3d *pCell, OdGeExtents3d& extBrep) const;

  // return internal brep interface of element
  virtual OdResult getBrep(const OdDgElement* pElement, OdDgModelerGeometryPtr &model) const;
};
