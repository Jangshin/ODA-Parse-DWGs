...
private:
  ...
  public:
    ...
    {
      A::copyConstruct(items(_size), value);
      return ++_size;
    }
    size_type insertAt(size_type i, const T& value)
    {
      ...
      A::copyAssignRange(items(i+1), items(i), moved);
      A::copyConstruct(items(i), value);
      return ++_size;
    }
    void resize(size_type newSize, const T& value)
    {
      ODA_ASSERT(newSize>_size);
      A::copyConstructFill(items(_size), newSize-_size, value);
      _size = newSize;
    }
    void resize(size_type newSize)
    {
      if(newSize>_size) {
        A::defaultConstructFill(items(_size), newSize-_size);
        _size = newSize;
      }
      else {
        A::destroyRange(items(0), _size - newSize);
        _size = newSize;
      }
    }
    void moveFrom(PAGE* src, size_type start)
    {
      ...
      A::copyAssignRangeDisjoint(items(_size), src->items(), start);
      _size += start;

      src->_size -= start;
      A::copyAssignRange(src->items(), src->items(start), src->_size);
    }
    void remove(size_type i)
    {
      A::destroyRange(items(i), 1);
      --_size;
      if ( i < _size )
        A::copyAssignRange(items(i), items(i+1), _size - i);
    }
    void removeLast()
    {
      A::destroyRange(items(_size--), 1);
    }
