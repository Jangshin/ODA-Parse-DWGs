...
public:
  ...
  double getFirstLineIndentation() const;
  void   setFirstLineIndentation( double dValue );

  double getParagraphIndentation() const;
  void   setParagraphIndentation( double dValue );

  OdDoubleArray getTabStopArray() const;
  void          setTabStopArray( const OdDoubleArray& arrTabStop );

  double  getDefaultTabStep() const;

  // Symbols before of first text
  OdArray<OdDgTextExtendedProperties::OdDgSpecialSymbol> getTextNodeStartSpecialSymbols() const;
  void setTextNodeStartSpecialSymbols( const OdArray<OdDgTextExtendedProperties::OdDgSpecialSymbol>& arrSymbols );
  ...
  void recalculateTextNode();

  virtual bool        getAnnotationScaleFlag() const;
  virtual void        setAnnotationScaleFlag(bool bSet, double dAnnotationScale = 0, bool bRescaleGeometry = true, bool doSubents = true);

  virtual double      getAnnotationScale() const;
  virtual void        setAnnotationScale(double dScale, bool bRescaleGeometry = true, bool doSubents = true);
  ...
  virtual void transformTextNodeSettings(const OdGeMatrix3d& matTransform);
...
#define TEXT_FLAGS_DEF(name)              \
  ...
  virtual void setColorIndex( OdUInt32 var, bool doSubents = false);
