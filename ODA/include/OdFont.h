...
struct FIRSTDLL_EXPORT OdTextProperties
{
  ...
  enum
  {
    ...
    kForMTextExtents = 0x2000
  };
  /** Previous character */
  OdChar  m_prevChar;
public:
  OdTextProperties() : m_trackingPercent(0.0), m_flags(0), m_textQuality(50), m_prevChar(0) {}
  ...
  */
  ...
  bool isForMTextExtents() const { return GETBIT(m_flags, kForMTextExtents); }
  void setForMTextExtents(bool value) { SETBIT(m_flags, kForMTextExtents, value); }
};
...
public:
  ...
  */
  ...
  /** \details
    ...
    \param character [in]  Character value.
    \param height    [in]  Character height.
    \param additionalThickness [in] Additional character thickness.
    \param textProperties [in] Text properties.
    \returns texture for character if successfully, or null otherwise.
  */
