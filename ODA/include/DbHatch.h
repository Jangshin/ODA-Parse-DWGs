...
public:
  ...
  */
  ...
  /** \details
    ...
    \param bCheckLoop [in]  Checking the loop for closure non-solid hatch.
    
    \remarks

    Checking the loop for closure is always on in ARX.
    Checking the loop for closure are enabled always for solid hatch.
  ...
  */
  ...
  /** \details
    ...
    \param originPoint [in]  Origin point.
  ...
  */
  void setPattern(
    ...
    OdGePoint2d originPoint = OdGePoint2d());
...
/** \details
   Adds loops to an OdDbHatch with an entity in the visible area of the viewport around the pick point.
    ...
    \param pDb             [in] the database from which the entities will be obtained to build a closed loops.
    \param pHatch          [in] the OdDbHatch to append new created loops from pick point. 
    \param pickPoint       [in] the point around which the nearest contour will be searched.
    \param hatchPlane      [in] the creation of a contour will be only between those entities that place in the hatchPlane plane.
    \param useInnerRegions [in] Set true if need create closed loops inner of loop created around pick point. False if create just one loop around pick point.
                                E.g. "if this parameter is set then inner islands will be created inside the outer boundary defined by the pick point".
    \returns eOk if successful.
             eNotCurrentDatabase if the pHatch has a database and this database differ from pDb. 
             eInvalidInput if the pDb or pHatch is null.
             eGeneralModelingFailure if can't create a loops from entities.
  */
