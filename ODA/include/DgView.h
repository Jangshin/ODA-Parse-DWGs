...
public:
  ...
  // current acs settings.

  virtual OdDgElementId getAcsId() const;
  virtual void          applyAcs( const OdDgElementId& idAcsElement );
  virtual void          applyAcs( const OdDgACS* pAcs);

  virtual OdGePoint3d   getAcsOrigin() const;
  virtual void          setAcsOrigin( const OdGePoint3d& ptOrigin );

  virtual OdGeMatrix3d  getAcsRotation() const;
  virtual void          setAcsRotation( const OdGeMatrix3d& matRotation );

  virtual bool          getAcsViewIndependentFlag() const;
  virtual void          setAcsViewIndependentFlag( bool bSet );

  virtual OdDgModel::AcsType getAcsType() const;
  virtual void               setAcsType( const OdDgModel::AcsType& uType );
  ...
  virtual void zoomToExtents( double dExtraExtentsFactor = 1.1, OdUInt32 uExtentFlags = 0);
};
...
public:
  ...
  OdDgElementId getAt(OdUInt32 uViewIndex) const;
  ...
  /** \details
    Return display order of views with indexes from 0 to 7.

  \remarks
    Binary data contains 8 bytes with view indexes (from 0 to 8).
    Top view is the last.
  */
  OdBinaryData getViewDisplayOrder() const;

  /** \details
    Set display order of views with indexes from 0 to 7.

  \param orderData[in]  array of 8 bytes with view indexes.

  \remarks
    Binary data must contains 8 bytes with view indexes. Indexes
    muts be unique values from 0 to 7.
  */
  OdResult     setViewDisplayOrder(const OdBinaryData& orderData);
...
public:
  ...
  */
  virtual OdDgElementId createViewGroupForModel( OdDgModelPtr pModel );
};
