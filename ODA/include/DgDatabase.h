...
class OdDgCompressDatabaseResult;
class OdDgDatabase;
...
namespace OdDg
{
  ...
  */
  ...
  enum OdDgCompressDatabaseFlags
  {
    ...
    kRemoveNullModelViewGroups     = 0x2000
  };

  enum OdDgOriginalFileFormat
  {
    kUnknownFormat  = 0,
    kDgnV7          = 1,
    kDgnV8          = 2,
...
/** \details
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgDatabase : public OdDgElement
{
...
public:
  ...
  */
  ...
  OdError if the write is unsuccessful.
  */
  void readFile( OdStreamBufPtr& pStream, Oda::FileShareMode shareMode = Oda::kShareDenyWrite, bool partialLoad = false );
  ...
  OdError if the write is unsuccessful.
  */
  void writeFile( OdStreamBufPtr& pStream, bool bSaveThumbnailImage = true );
  ...
  */
  ...
  \param filename [in]  Name of file.
  \param dibData [in] Binary data of thumbnail

  */
  static bool setThumbnail(
    const OdString& filename, 
    const OdBinaryData& dibData );

  bool getThumbnail( OdBinaryData& dibData );

  bool setThumbnail( const OdBinaryData& dibData );
  ...
  */
  bool getRetainOriginalThumbnailBitmap() const;
  ...
  */
  ...
  virtual OdDgElementId ownerId() const;
...
*/
  virtual OdDgElementId elementId() const;
  ...
  */
  ...
  \param reactors [out]  Receives this database's reactors.
  */
  void getReactors( 
    OdDgDatabaseReactorArray& reactors);
  ...
  */
  ...
  void setPropertiesFrom(const OdDgDatabase* pEntFrom, bool doSubents = true);
  ...
  */
  ...
  \param idBlockDefOrModel [in]  Element ID of the SharedCellDefinition or OdDgModel to be cloned.

  \remarks
  The method creates a new OdDgDatabase object, populates it with specified objects from this database using the wblockClone() method and returns a smart pointer to the new database.
...
*/
  OdDgDatabasePtr wblock(
    const OdDgElementId& idBlockDefOrModel);

  /** \details
    Clones the entire contents of this database to a new database.

    \remarks
    The method creates a new OdDgDatabase object, populates it with objects from this database and returns a smart pointer to the new database.
    All objects are copied with their dependencies. For example, if a text entity uses a specific textstyle, it will be copied as well; line entities will be copied with their linestyles, etc.
  */
  ...
  /** \details
    Clones set of objects of this database to a new database.

    \remarks
    The method creates a new OdDgDatabase object, populates it with set of objects from this database and returns a smart pointer to the new database.
    All objects are copied with their dependencies. For example, if a text entity uses a specific textstyle, it will be copied as well; line entities will be copied with their linestyles, etc.
  */

  OdDgDatabasePtr wblock(const OdDgElementIdArray& objectIds, const OdGePoint3d& basePoint);
  ...
  */
  ...
  /** \details
    ...
    At least 1 undo step is always supported by ODA Software for internal needs.
  */
  void undo();
...
//  void disableUndoRecording(bool bDoIt);
  ...
  */
  ...
  virtual OdDg::OdDgOriginalFileFormat  getOriginalFormat() const;
  virtual void      setOriginalFormat( OdDg::OdDgOriginalFileFormat val );

  virtual bool     getLayoutDataFromSheetModelPaperSettingsFlag() const;
  virtual void     setLayoutDataFromSheetModelPaperSettingsFlag( bool bSet );
  ...
  */
  virtual void compressDatabase( const OdDg::OdDgCompressDatabaseFlags& compressFlags, OdDgCompressDatabaseResult* pResult = NULL);

  /** \details
    Set double internal line style dash scale if it can't be applied during database ceration.
    Default mode are stored in DG_DOUBLE_INTERNAL_LINESTYLE_SCALE variable of dgn host app services.
  */

  virtual void setDoubleInternalLineStyleDashScale( bool bSet );
...
/** \details
This class defines container and tool to customize compress functionality.
...
/** \details
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgCompressDatabaseResult
{
public:
  OdDgCompressDatabaseResult();
  virtual ~OdDgCompressDatabaseResult();

  virtual void addCompressedElement(const OdDgElementId& idElement, const OdDg::OdDgCompressDatabaseFlags& condition);
  virtual bool allowToEraseElement(const OdDgElementId& idElement, const OdDg::OdDgCompressDatabaseFlags& condition);

  virtual void eraseAll();
  virtual void clear();

  virtual bool      hasElementId(const OdDgElementId& idElement) const;
  virtual OdUInt16  getCondintionOfErase(OdDgElementId& idElement) const;

  virtual OdUInt32  getCount() const;
  virtual bool      getRecord(OdUInt32 uIndex, OdDgElementId& idElement, OdUInt16& uCondition) const;
  virtual bool      removeRecord(OdDgElementId& idElement);
  virtual bool      removeRecords(const OdDg::OdDgCompressDatabaseFlags& conditionFlags, bool bCheckOtherConditions = false);

private:
  std::map<OdDgElementId, OdUInt16> m_mapCompressedElements;
};
