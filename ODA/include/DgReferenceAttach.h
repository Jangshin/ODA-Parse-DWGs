...
public:
  ...
  void addClipPoint(const OdGePoint2d& point);
  void deleteClipPoint(OdUInt32 index);

  bool         getClipRotationMatrix(OdGeMatrix3d& matRotation) const;
  void         setClipRotationMatrix(const OdGeMatrix3d& matRotation, bool bRemoveRotation = false);
  ...
  virtual bool getManipulateAsElementFlag() const;
  virtual void setManipulateAsElementFlag(bool val);
  ...
  virtual bool getUseLevelFlag() const;
  virtual void setUseLevelFlag(bool bSet);
  ...
  void                      setForeignUnits( OdDgReferenceForeignUnits iUnits );
  ...
  virtual OdResult subGetGeomExtents(OdUInt32 uExtentFlags, OdGeExtents3d& extents) const;
  ...
  // The method set Display flag and load reference attachment again.
  virtual void reload(bool bShowReference = true);

  // The method reset Display flag and unload referenced database from memory.
  virtual void unload(bool bHideReference = true);
  virtual bool getUnloadFlag() const;
...
public:
  ...
  static bool extractBoundary( OdDgReferenceAttachmentHeaderPtr reference, OdGiClipBoundary& clipBoundary, double deviation,
    bool bViewIndependedClipping, OdGiCommonDraw* pWd, bool& bClipIsInverce, bool bSetDeviation = false, const OdGeMatrix3d& matXRefTransform = OdGeMatrix3d::kIdentity,
  ...
  static bool extractBoundary( const OdDgReferenceAttachmentHeaderPtr& reference, OdGiClipBoundary& clipBoundary, 
                               ...
                               OdGiCommonDraw* pWd, bool bSetDeviation = false, const OdGeMatrix3d& matXRefTransform = OdGeMatrix3d::kIdentity );
...
<group OdDg_Classes>
*/
class TG_EXPORT OdDgReferenceLayout : public OdDgElement
{
public:
  ODDG_DECLARE_MEMBERS_INTERNAL(OdDgReferenceLayout);
public:
  OdDgElementId getReferenceId() const;
  virtual void setReferenceId(const OdDgElementId& idReferenceAttach);
  //overrides for OdGiDrawable
  bool subWorldDraw(OdGiWorldDraw* pWd) const;
  void subViewportDraw(OdGiViewportDraw *pVd) const;
  OdUInt32 subSetAttributes(OdGiDrawableTraits* pTraits) const;
...
typedef OdSmartPtr< OdDgReferenceLayout > OdDgReferenceLayoutPtr;
...
/** \details
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgReferenceAttachCDAPE : public OdRxObject
{
  ODRX_DECLARE_MEMBERS(OdDgReferenceAttachCDAPE);

  long m_references;
public:

  OdDgReferenceAttachCDAPE() {};
  ...
  virtual OdDgElementId getModelIdForPropertyCollections( OdDgElement* pXRef ) const;
};

//============================================================================================

typedef OdSmartPtr<OdDgReferenceAttachCDAPE> OdDgReferenceAttachCDAPEPtr;
...
/** \details
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgDwgReferenceAttachmentPE : public OdRxObject
{
public:
  ODRX_DECLARE_MEMBERS(OdDgDwgReferenceAttachmentPE);

  //load/render issues
  virtual OdRxObjectPtr readFile(const OdDgDatabase* pDb, const OdString& fileName,
    Oda::FileShareMode shareMode = Oda::kShareDenyWrite, bool bMTLoading = false) = 0;

  virtual bool worldDraw(const OdDgDatabase* pDb, OdRxObjectPtr dwgDatabase, const OdString& strLayoutName, bool bShowLineWeight = true, OdGiCommonDraw* pWd = NULL, OdDgLevelTable* pLevelTable = NULL) = 0;
  virtual void getGeomExtents(OdRxObjectPtr dwgDatabase, OdGeExtents3d& extents) = 0;
  virtual double getScale(const OdDgDatabase* pDatabase, OdRxObjectPtr dwgDatabase, double dDefaultUnitScale = 1) const = 0;

  virtual void createLevelTableForDwgDatabase(OdRxObjectPtr& pDwgDb, OdDgLevelTablePtr& pDgLevelTable) = 0;
  virtual void updateLayerTableByLevelTable(OdRxObjectPtr& pDwgDb, const OdDgLevelTablePtr& pDgLevelTable, const OdDgElementId& idLevelMask) = 0;
};

/** \details
  This template class is a specialization of the OdSmartPtr class for OdDgDwgReferenceAttachmentPE object pointers.
*/

typedef OdSmartPtr<OdDgDwgReferenceAttachmentPE> OdDgDwgReferenceAttachmentPEPtr;
