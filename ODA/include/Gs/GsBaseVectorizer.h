...
/** \details
  The data type that represents a shared pointer to the <link OdGsApplyModelOverrides, OdGsApplyModelOverrides> object.
*/
...
public:  
  ...
  */
  ...
  virtual void reportUpdateError(OdDbStub* /*entityId*/, const OdError& error)
  {
    ...
    throw error;
#else
    ODA_VERIFY(error); // prevent arg unused warning
#endif
  }
  ...
  */
  ...
  /** \details
    Sets up subentity marker transformation state.
    \param bEnableTf [in]  Enables transformation state.
    \param refMarker [in]  Reference marker value.
    \param pTfBranch [in]  Transformation branch.
  */
  void setSubentityTransform(bool bEnableTf, OdGsMarker refMarker = kNullSubentIndex, const OdGsStateBranch *pTfBranch = NULL);

  /** \details
    Applies subentity marker transformation state.
    \param pXform [in]  If not null - push transfomration matrix, or pop elsewhere.
  */
  ...
  */
  ...
  /** \details
    Returns a pointer to a subnode branch of the top node.
    \param branchType [in] Type of state branch.
    \returns
  ...
  */
  ...
  /** \details
    Returns current state branch.
    \param branchType [in] Type of state branch.
    \returns
    Pointer to the current state branch.
  */
  ...
  */
  ...
  /** \details
    Returns current vectorization thread index.
    \returns
    Thread index.
  */
...
protected:
  ...
  virtual void doRayTrace(const OdGePoint3d &rayOrigin, const OdGeVector3d &rayDirection,
                          OdGsRayTraceReactor *pReactor, bool bSortedSelection,
                          OdGiPathNode const *const *pObjectList, OdUInt32 nObjectListSize);
...
private:
  ...
  //DOM-IGNORE-END
public:
  OdGsStateBranchPtr m_pCurStateBranch[OdGsStateBranch::kNumBranchTypes]; // Current state branches
...
protected:
  ...
  enum BaseVectorizerFlags
  {
    ...
    kSubMarkerXformSet      = OdGiBaseVectorizerImpl::kLastImplFlag << 6,

    kLastBaseVectorizerFlag = kSubMarkerXformSet
  };
...
inline const OdGsStateBranch *OdGsBaseVectorizer::currentStateBranch(OdGsStateBranch::BranchType branchType) const
{
  return m_pCurStateBranch[branchType];
}

inline bool OdGsBaseVectorizer::hasAnyStateBranch(bool bMarkers) const
{ bool bHasAnyBranch = false;
  for (OdUInt32 nBranch = 0; nBranch < OdGsStateBranch::kNumBranchTypes; nBranch++)
  { if (!m_pCurStateBranch[nBranch].isNull() && (!bMarkers || !m_pCurStateBranch[nBranch]->markersEmpty()))
    { bHasAnyBranch = true; break; } }
  return bHasAnyBranch;
}
