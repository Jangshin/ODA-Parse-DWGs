...
public:
  ...
  void setInvalid(OdUInt32 nVpID)
  {
    if (m_vpAwareFlags.size() > nVpID)
      m_vpAwareFlags[nVpID] = 0xFFFFFFFF;
  }
...
public:
  ...
  enum
  {
    ...
    kBrOwnGeom            = OdGsEntityNode::kLastFlag << 5,
    kLastFlag             = kBrOwnGeom
  };
  ...
  */
  ...
  /** \details
      ...
      \param transform [out] A transformation.
      \returns True if block reference is shared and transformation was successfully retrieved, or false otherwise.
  */
  ...
  /** \details
      ...
      \param mask     [in]  Mask.
      \param bRecursively [in] A flag that indicates whether nested reference should invalidate cache nodes or not.
  */
  virtual void invalidateBlockNode( OdGsContainerNode* pParent,
    OdGsViewImpl* pView, OdUInt32 mask, bool bRecursively = true );
...
protected:
  ...
  {
    if(!m_pImpl.get() || !m_pImpl->isSharedReference())
      return m_pImpl;
    return getSharedImpl(view);
  }
...
private:
  ...
  void clearImpl() { m_pImpl = NULL; }
  bool validateSharedRefImpl(const OdGsUpdateContext &ctx, bool &bTryShared);
  OdGsBlockReferenceNodeImpl *getSharedImpl(const OdGsViewImpl &view);
protected:
