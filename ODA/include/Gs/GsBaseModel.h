...
protected:
  ...
  typedef std::map<OdUInt32, OdGsIntParamPtr> SelectionStyleParamMap;
...
public:
  ...
  */
  ...
  /** \details
    ...
    \param pView [in]  Pointer to the view where the specified entity should be transformed.
  */
  virtual void externalTransformImpl(const OdGiPathNode& path, const OdGsMarker* pMarkers, OdUInt32 nMarkers, bool bDoIt = true,
                                     const OdGsMatrixParam *xForm = NULL, const OdGsView* pView = NULL);
  ...
  */
  ...
  /** \details
    Gets sectioning parameters of the model. 
    
    \returns
    Instance of SectioningSettings structure containing sectioning parameters.
    \sa
    <link tv_working_with_features_cutting_planes, Work with Cutting Planes>
    \sa
    <link tv_planarClip_classes, Main Classes for Planar Clipping Section Generation>
