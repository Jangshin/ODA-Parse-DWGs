...
public:
  ...
  */
  ...
  /** \details
    Enables including of view object frozen layers visibility in view extents.

    \param bEnable [in] Enables or disables frozen layers visibility inclusion.
  */
  void includeFrozenLayersVisibilityInViewExtents(bool bEnable);

  /** \details
    Returns true if view object frozen layers visibility should be included in view extents.

    \returns
    true if view object frozen layers should be included in view extents.
  */
...
public:
  ...
  */
  inline bool hasLweights() const { return !m_lweights.empty() || !m_lweights2x.empty(); }
  
  /** \details
    Returns number of lineweights in lineweights array.
    \returns
    Array length if lineweights is not empty, or zero otherwise.
  */
  inline OdUInt32 numLweights() const { return (m_lweights2x.empty()) ? m_lweights.size() : m_lweights2x.size(); }

  /** \details
    Returns lineweight from lineweights array.
    \returns
    Lineweight.
  */
  inline OdUInt32 getLweight(int nLineweight) const { return (m_lweights2x.empty()) ? OdUInt32(m_lweights[nLineweight]) : OdUInt32(m_lweights2x[nLineweight]); }
  ...
  */
  ...
  /** \details
    Retrieves an array of lineweights in alternative format.
  */
  inline const OdUInt16Array &getLweightsEnum2() const { return m_lweights2x; }
...
public:
  ...
  */
  ...
  /** \details
  If show frozen layers flag is true then kDrawLayerOff|kDrawLayerFrozen flags will be force set before of
  vectorization.
  \returns
  true if show frozen layers mode is on, false otherwise.
*/
  ...
  If show frozen layers flag is true then kDrawLayerOff|kDrawLayerFrozen flags will be force set before of
  vectorization.
  \param bSet [in] show frozen layers mode status.
*/
...
private:
  ...
  enum EGsViewImplFlags
  {
    ...
    kFrozenLayersInViewExt   = kFirstViewImplFlag << 20,
    kShowFrozenLayers        = kFirstViewImplFlag << 21
  };
protected:
...
inline void OdGsViewImpl::includeFrozenLayersVisibilityInViewExtents(bool bEnable)
{
  SETBIT(m_gsViewImplFlags, kFrozenLayersInViewExt, bEnable);
}

inline bool OdGsViewImpl::isFrozenLayersVisibilityIncludedInViewExtents() const
{
  return GETBIT(m_gsViewImplFlags, kFrozenLayersInViewExt);
}
...
inline bool OdGsViewImpl::isShowFrozenLayers()
{
  return GETBIT(m_gsViewImplFlags, kShowFrozenLayers);
}

inline void OdGsViewImpl::setShowFrozenLayers(bool bSet)
{
  SETBIT(m_gsViewImplFlags, kShowFrozenLayers, bSet);
}
