...
public:
  /** \details
    Destructor for the OdGsNode class.
  */
  virtual ~OdGsNode();
  ...
  */
  ...
  /** \details
    ...
    \param ext   [out]  Reference to the OdGeExtents3d object which will receive current node extents.
  */
  virtual bool extents(const OdGsView* pView, OdGeExtents3d& ext) const;

  /** \details
    Returns the extents of the cached geometry optionally transformed by GsModel transformation matrix.
    \param ext [out] Receives the extents.
    \param bUseModelTf [in] Apply GsModel transformation matrix to received extents.
    \returns
    Returns true if and only if the cached geometry have extents.
  */
  bool modelExtents(OdGeExtents3d& ext, bool bUseModelTf = true) const;
  /** \details
    Returns the extents of the cached geometry for specified view optionally transformed by GsModel transformation matrix.
    \param pView [in] View object to receive extents for.
    \param ext [out] Receives the extents.
    \param bUseModelTf [in] Apply GsModel transformation matrix to received extents.
    \returns
    Returns true if and only if the cached geometry have extents for specified view.
  */
  bool modelExtents(const OdGsView* pView, OdGeExtents3d& ext, bool bUseModelTf = true) const;

  /** \details
    Retrieves a maximal lineweight of geometry contained by this node.
    \returns
