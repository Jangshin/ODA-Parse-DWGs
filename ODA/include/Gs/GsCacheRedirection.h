...
class OdGsModelRedirectionWrapper : public Base
{
  ...
  public:
    ...
    */
    ...
    /** \details
      Applies external transformation to the specified entity in this Model object.
    
      \param path [in]  Path to the entity for transformation setting.
      \param bDoIt [in]  Flag that indicates whether transformation is enabled (true value) or disabled (false value).
      \param xForm [in]  Transformation matrix.
      \param pView [in]  Pointer to the view where the specified entity should be transformed.
    */
    virtual void externalTransform(const OdGiPathNode& path, bool bDoIt = true, const OdGsMatrixParam *xForm = NULL,
                                   const OdGsView* pView = NULL)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
        m_pRedirectionClient->gsModelAt(nModel)->externalTransform(path, bDoIt, xForm, pView);
    }

    /** \details
      Transforms the specified subentities of entity in this Model object..
      ...
      \param pView [in]  Pointer to the view where the specified entity should be transformed.
    */
    virtual void externalTransform(const OdGiPathNode& path, const OdGsMarker* pMarkers, OdUInt32 nMarkers, bool bDoIt = true,
                                   const OdGsMatrixParam *xForm = NULL, const OdGsView* pView = NULL)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
        m_pRedirectionClient->gsModelAt(nModel)->externalTransform(path, pMarkers, nMarkers, bDoIt, xForm, pView);
    }

    /** \details
      Returns external transform if exists the specified subentities of entity in this Model object.

      \param path [in]   Path to the entity for getting transformation.
      \param xForm [out] Result transformation matrix.
      \param pView [in]  Pointer to the view where to find specified entity transformation.
    */
    virtual bool getExternalTransform(const OdGiPathNode &path, const OdGsMatrixParam *&xForm, const OdGsView *pView = NULL) const
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
      {
        if (m_pRedirectionClient->gsModelAt(nModel)->getExternalTransform(path, xForm, pView))
          return true;
      }
      return false;
    }
...
class GS_TOOLKIT_EXPORT OdGsModelRedirectionHandler : public OdGsRedirectionHelper<OdGsModelRedirectionWrapper<OdGsModelRedirectionHandler> >
{
  ...
  public:
    /** \details
      Default constructor for the OdGsModelRedirectionHandler class.
    */
    OdGsModelRedirectionHandler() : m_nModels(0), m_nReferences(0), m_pManager(NULL) { setRedirectionClient(this); }
    
    /** \details
      Destructor for the OdGsModelRedirectionHandler class.
    */
...
class GS_TOOLKIT_EXPORT OdGsCacheRedirectionNode : public OdGsRedirectionHelper<OdGsCache>
{
  ...
  public:
    /** \details
      Default constructor for the OdGsCacheRedirectionNode class.
    */
    OdGsCacheRedirectionNode() : m_pModels(NULL) { }
    
    /** \details
      Destructor for the OdGsCacheRedirectionNode class.
    */
...
class GS_TOOLKIT_EXPORT OdGsCacheRedirectionManager : public OdRxObject
{
  ...
  public:
    /** \details
      Default constructor for the OdGsCacheRedirectionManager class.
    */
    OdGsCacheRedirectionManager() : m_nReferences(0), m_pFinderEntry(NULL) { }
    
    /** \details
      Destructor for the OdGsCacheRedirectionManager class.
    */
