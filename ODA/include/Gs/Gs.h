...
public:
  ...
  */
  enum SelectionMode
  {
    ...
    kPointLast    = 17  // Same as kPoint, but for 2D render mode it returns last selected object.
  };
  ...
  */
  ...
  /** \details
    ...
    \param altSourceLwds [in]  Array of Lineweights in alternative source format.
    \remarks
    There is no reason to specify both arrays at same time. In case if both arrays specified, alternative source
    lineweights will be used.
    \sa
    <link tv_working_with_attributes_lineweights, Work with Lineweights>
  */
  ...
  */
  ...
  /** \details
    Retrieves a smart pointer to the root drawable at the specified index.
    \param i [in]  Index of the drawable to return.
    \param pModelReturn [in/out]  If not NULL, receives a pointer to model of associated drawable holder.
  */
  ...
  */
  ...
  /** \details
    Runs ray tracing procedure inside this Viewport object.
    ...
    \param pReactor [in]  Ray tracing callback reactor.
  ...
  */
  virtual void rayTrace(const OdGePoint3d &rayOrigin, const OdGeVector3d &rayDirection,
                        OdGsRayTraceReactor *pReactor, bool bSortedSelection = false,
                        OdGiPathNode const* const* pObjectList = NULL, OdUInt32 nObjectListSize = 0) = 0;
...
struct OdGsClientViewInfo 
{
  ...
  /** \details
    Default constructor for the OdGsClientViewInfo class.
  */
  OdGsClientViewInfo()
    : viewportId(-1)
    , acadWindowId(0)
