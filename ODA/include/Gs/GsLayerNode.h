...
/** \details
  The data type that represents a vector of the <link OdGiLayerTraitsData, OdGiLayerTraitsData> objects.
*/
...
protected:
  ...
  */
  ...
  mutable OdGiLayerTraitsDataArray m_vpFrozenTraits;
...
public:
  ...
  */
  ...
  /** \details
    ...
    \param bVpFrozen [in]  Take viewport-frozen layer frags into account.
  */
  ...
  /** \details
    ...
    \param bVpFrozen [in]  Take viewport-frozen layer frags into account.
  */
  const OdGiLayerTraitsData& layerTraits(OdUInt32 nVpId, bool bVpFrozen = true) const;
...
inline bool OdGsLayerNode::isUpToDate(OdUInt32 nVpId) const
{
  ...
  if(isVpDep())
  {
    if(nVpId > 0 && m_vpLayerTraits.size() < nVpId)
      return false;
    return !GETBIT(layerTraits(nVpId, false).flags(), 0x80000000);
  }
  return !GETBIT(layerTraits(0, false).flags(), 0x80000000);
}
...
inline OdGiLayerTraitsData& OdGsLayerNode::layerTraits(OdUInt32 nVpId, bool bVpFrozen)
{
  return const_cast<OdGiLayerTraitsData&>(static_cast<const OdGsLayerNode*>(this)->layerTraits(nVpId, bVpFrozen));
}

inline const OdGiLayerTraitsData& OdGsLayerNode::layerTraits(OdUInt32 nVpId, bool bVpFrozen) const
{
  if (isVpFrozen() && bVpFrozen && m_vpFrozenTraits.size() > nVpId && m_vpFrozenTraits[nVpId].m_flags != 0xFFFFFFFF)
    return m_vpFrozenTraits[nVpId];
  if(isVpDep() && nVpId > 0)
  {
