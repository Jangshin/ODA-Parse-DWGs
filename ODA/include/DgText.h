...
public:
  ...
  virtual double getFirstLineIndentationOverride() const;
  virtual void   setFirstLineIndentationOverride( double dValue );

  virtual double getParagraphIndentationOverride() const;
  virtual void   setParagraphIndentationOverride( double dValue );

  virtual OdDoubleArray getTabStopArrayOverride() const;
  virtual void          setTabStopArrayOverride( const OdDoubleArray& arrTabStop );

  virtual bool   getTextIndentationOverrideFlag() const;
  virtual void   setTextIndentationOverrideFlag( bool bSet );

  virtual bool   getEndOfLineFlag() const;
  virtual void   setEndOfLineFlag( bool bSet );

  virtual OdArray<OdDgTextExtendedProperties::OdDgSpecialSymbol> getSpecialSymbolsAfterText() const;
  virtual void setSpecialSymbolsAfterText( const OdArray<OdDgTextExtendedProperties::OdDgSpecialSymbol>& arrSymbols );
  ...
  virtual void getBoundingPoints(OdGePoint2dArray& boundingPoints) const;
...
#define TEXT_FLAGS_DEF(name)              \
  ...
  virtual bool getUpsideDownFlag() const;
  ...
  virtual void        setColorIndex( OdUInt32 color, bool doSubents = false);
...
*/
  ...
  virtual bool applyTextStyle( OdDgElementId idTextStyle, bool bActiveModelUnitsForNonDBRO = true, double dActualUORsToMasterScale = -1.0);
...
<group OdDg_Classes>
*/
class TG_EXPORT OdDgTextMirrorControlPE : public OdRxObject
{
  ODRX_DECLARE_MEMBERS(OdDgTextMirrorControlPE);
  long        m_references;     //used by each object separately
public:

  OdDgTextMirrorControlPE();
  ~OdDgTextMirrorControlPE();
  ...
  virtual bool getMirrorTextFlag(const OdDgGraphicsElement* pText) const;
};

typedef OdSmartPtr<OdDgTextMirrorControlPE> OdDgTextMirrorControlPEPtr;
