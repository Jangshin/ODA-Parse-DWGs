...
public:
  ...
  */
  ...
  {
    return OdSmartPtr<TInterface>(static_cast<TInterface*>(new OdRxObjectImpl<T, TInterface>(std::forward<Args>(args)...)), kOdRxObjAttach);
  }
};

// VS 2013 does support arguments forwarding but does not quite support `using`
#if (defined(_MSC_VER) && (_MSC_VER > 1800)) || (defined(__cplusplus) && (__cplusplus > 199711L))
template<class T, class TInterface = T> using OdRxObjectImpl2 = OdRxObjectImpl<T, TInterface>;
#else
