...
#include "OdPointCloudDatabaseReceiver.h"

#include "OdPointCloudProjectScanStorage.h"
...
public:
  ...
  */
  ...
  /** \details
    Creates a new point cloud scan database object and populates it with the
    data read from a point cloud scan file (.rcs file) represented by stream.

    \param pFile [in]  Stream corresponding to .rcs file.
  ...
  */
  virtual OdPointCloudProjectDatabasePtr readRcpFile(const OdString& filePath, OdPointCloudProjectScanStorage* pScanStorage, OdPointCloudDatabaseReceiver* pDbReceiver = NULL) const = 0;

  /** \details
    Creates a new point cloud project database object and populates it with the
    data from the corresponding point cloud project file (.rcp file) represented by stream.

    \param pFile [in] Stream corresponding to .rcp file.

    \returns
    The pointer to the OdPointCloudProjectDatabase object.
  */
  virtual OdPointCloudProjectDatabasePtr readRcpFile(OdStreamBufPtr pFile, OdPointCloudProjectScanStorage* pScanStorage, OdPointCloudDatabaseReceiver* pDbReceiver = NULL) const = 0;
