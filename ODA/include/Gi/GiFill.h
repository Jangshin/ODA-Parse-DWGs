...
class FIRSTDLL_EXPORT OdGiHatchPattern : public OdGiFill
{
  protected:
    ...
    enum Flags { kDraft = (1 << 0), kExternal = (1 << 1), kAlignedToCenter = (1 << 2), kEmpty = (1 << 3), kSolid = (1 << 4), kDisabledBackgroundFill = (1 << 5)};
    OdUInt32 m_nFlags;
    //DOM-IGNORE-END
  public:
    ...
    */
    ...
    \returns 'kSolid' flag value
    */
    bool isSolid() const;

    /** \details
    Is used to set 'kSolid' flag that forces 
    using a solid fill instead of pattern while rendering
    \param empty [in] Value to be set
    */
    ...
    \returns 'kDisabledBackgroundFill' flag value
    */
    bool isDisabledBackgroundFill() const;

    /** \details
    Is used to set 'kDisabledBackgroundFill' flag that disables solid bg fill
    on the same face rendering, so only pattern and edges are drawn
    \param empty [in] Value to be set
    */
...
inline OdGiFillHatch::OdGiFillHatch() : OdGiHatchPattern(),
