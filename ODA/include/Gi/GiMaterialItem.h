...
class ODGI_EXPORT OdGiMaterialTextureData : public OdRxObject
{
  ...
  public:
    ...
    /** \details
      Called when material texture data unlinked from textures manager and material item.
      \param bReason [in]  True means unlink from textures manager and false for unlink unmanaged texture from material item.
    */
  ...
  public:
    ...
    */
    ...
    /** \details
      ...
      \param pOrder [in]  Encoded order of texture channels (see remarks).
      \param bCheckAny [in]  Stops check on first available texture and return true in this case.
      \returns  Set of bits for enabled texture channels.
      \remarks  Encoded texture channels consist from two characters for each: "df" for diffuse channel,
                "oc" for opacity channel, "bm" for bump-map channel, "sc" for specular channel,
                "rf" for refraction channel, "es" for emission channel, "nm" for normal map channel,
                "rh" for roughness channel, "co" for cutouts channel, "rl" for reflection channel
                and "en" for environment channel.
    */
...
class ODGI_EXPORT OdGiMaterialRenderItem : public OdGiMaterialItem
{
  public:
    ...
    */
    ...
    /** \details
      Retrieves roughness texture pointer.
      \returns
      smart pointer to the roughness texture.
    */
    ...
    */
    ...
    /** \details
      Creates and returns roughness texture pointer.
      \returns
      smart pointer to the created roughness texture.
    */
    virtual OdGiMaterialTextureEntryPtr createRoughnessTexture() = 0;
    /** \details
      Destroys roughness texture.
    */
    virtual void removeRoughnessTexture() = 0;
    /** \details
      Checks whether the roughness texture is created.
      \returns
      true if this material item has an roughness texture, false otherwise.
    */
    virtual bool haveRoughnessTexture() const = 0;

    /** \details
      Retrieves cutouts texture pointer.
      \returns
      smart pointer to the cutouts texture.
    */
    ...
    */
    ...
    /** \details
      Creates and returns cutouts texture pointer.
      \returns
      smart pointer to the created cutouts texture.
    */
    virtual OdGiMaterialTextureEntryPtr createCutoutsTexture() = 0;
    /** \details
      Destroys cutouts texture.
    */
    virtual void removeCutoutsTexture() = 0;
    /** \details
      Checks whether the cutouts texture is created.
      \returns
      true if this material item has an cutouts texture, false otherwise.
    */
    virtual bool haveCutoutsTexture() const = 0;

    /** \details
      Retrieves environment texture pointer.
      \returns
      smart pointer to the environment texture.
    */
    ...
    */
    ...
    /** \details
      Creates and returns environment texture pointer.
      \returns
      smart pointer to the created environment texture.
    */
    virtual OdGiMaterialTextureEntryPtr createEnvironmentTexture() = 0;
    /** \details
      Destroys environment texture.
    */
    virtual void removeEnvironmentTexture() = 0;
    /** \details
      Checks whether the environment texture is created.
      \returns
      true if this material item has an environment texture, false otherwise.
    */
