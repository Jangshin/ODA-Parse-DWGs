...
public:
  ...
  //set/get ID
  OdUInt32  getID() const {return ID;}
...
{
  template <OdUInt32 NUM_AXIS, OdUInt32 MAX_DEPTH, OdUInt32 MAX_NODE_OBJECT, class EE, class P, class OO> friend class OdGiExtentsSpaceTree;
...
/** \details
  .
    ...
    <group OdGi_Classes>
*/
template <OdUInt32 NUM_AXIS, OdUInt32 MAX_DEPTH, OdUInt32 MAX_NODE_OBJECT, class E, class P, class O> class OdGiExtentsSpaceTree
{
  //root node
  ...
  //Is adaptive - means that started additioanl splitting when the number of objects starts to be greater than 'm_iMaxNodeObjects'
  bool              m_bIsAdaptive;
  OdUInt32          m_iMaxNodeObjects;
...
public:
  ...
  void setMaxNodeObjects(OdUInt32 nObjects) { m_iMaxNodeObjects = nObjects <= 0 ? m_iMaxNodeObjects : nObjects; }
...
class OdGiExtents3dSpacePoint : public OdGiExtentsSpaceObject
{
  ...
  std::set<OdUInt32> m_edges;

  // set of the IDs of invisible edges which have this vertex
  std::set<OdUInt32> m_invisibleEdges;
...
public:
  ...
  //constructor
  OdGiExtents3dSpacePoint(const OdGePoint3d& pt, OdUInt32 uniqueID) : OdGiExtentsSpaceObject(uniqueID)
  {
  ...
  const std::set<OdUInt32>* getEdges(){return &m_edges;}
  const std::set<OdUInt32>* getInvisilbeEdges(){return & m_invisibleEdges;}
...
public:
  ...
  //constructor
  OdGiExtents3dSpaceEdge(OdGiExtents3dSpacePoint& pt1, OdGiExtents3dSpacePoint& pt2, OdUInt32 uniqueID) : OdGiExtentsSpaceObject(uniqueID)
  {
    m_iVert1 = pt1.getID();
    m_iVert2 = pt2.getID();
...
public:
  ...
  //constructor
  OdGiExtents3dSpaceChainPolyline(OdUInt32 uniqueID) : OdGiExtentsSpaceObject(uniqueID){}
...
class OdGiExtents3dSpaceUtils
{
 ...
 static OdGiExtents3dSpacePoint* getNextStartVertex(OdArray<OdGiExtents3dSpacePoint*>& points, OdList<OdGiExtents3dSpacePoint*>& leafs)
 {
   ...
   {
     unsigned iVertNumber = points.size();
     for (unsigned i = 0; i < iVertNumber; i++)
     {
...
static OdGiExtents3dSpaceEdge* getFirstUnvisitedEdge(OdGiExtents3dSpacePoint* pVertex, OdArray<E*>& edges)
{
  ...
  const std::set<OdUInt32>* pEdges = pVertex->getEdges();
  if ( pEdges == NULL )
    return NULL;

  std::set<OdUInt32>::const_iterator itEdge = pEdges->begin();
  std::set<OdUInt32>::const_iterator itEdgeEnd = pEdges->end();
  while ( itEdge != itEdgeEnd )
  {
    OdUInt32 edgeID = *itEdge;
...
static OdInt64 calculateChainPolylinesFromEdges(OdArray<OdGiExtents3dSpacePoint*>& points, OdArray<E*>& edges, OdList<T*>& polylines )
{
  //1. Find the vertices with power 1 if exists
   ...
   OdUInt32 iVertNumber = points.size();
   for (OdUInt32 i = 0; i < iVertNumber; i++)
   {
     OdGiExtents3dSpacePoint* pVert = points[i];
     if ( pVert )
     {
