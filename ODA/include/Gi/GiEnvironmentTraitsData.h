...
public:
  ...
  /** \details
    Default constructor for the OdGiImageBackgroundTraitsData class. 
    Creates an instance with default options: background is not fit to screen, aspect ratio is not maintained, tiling is not used
    and background is an image-based background.
  */
...
public:
  ...
  */
  OdGiSkyBackgroundTraitsData()
    ...
    , m_visibilityDistance(0.)
    , m_diskScale(0.)
    , m_glowIntensity(0.)
    , m_diskIntensity(0.)
    , m_solarDiskSamples(0)
    , m_redBlueShift(0.)
    , m_saturation(0.)
  { m_type = OdGiDrawable::kSkyBackground; }
...
public:
  ...
  */
  OdGiIBLBackgroundTraitsData() 
    ...
    , m_bDisplay(false)
    , m_secBkgndId(NULL)
  { m_type = OdGiDrawable::kImageBasedLightingBackground; }
...
/** \details
    ...
    <group OdGi_Classes>
*/
class OdGiEnvironmentBackgroundTraitsData : public OdGiBackgroundTraitsData
{
  double m_longRot;
  double m_latRot;
  double m_fovAngle;
...
public:
  /** \details
    Default constructor for the OdGiEnvironmentBackgroundTraitsData class. Sets the 
    OdGiDrawable::kEnvironmentBackground type for this object.
  */
  OdGiEnvironmentBackgroundTraitsData() : m_longRot(0.0), m_latRot(0.0), m_fovAngle(OdaToRadian(45.0)), m_bFovOverride(false)
  { m_type = OdGiDrawable::kEnvironmentBackground; }

  /** \details
    Specifies longitude and latitude background rotation angles.
    
    \param longRad [in]  Longitude rotation angle in radians.
    \param latRad [in]  Latitude rotation angle in radians.

    \remarks
    Default rotation angles set to zeroes (no rotation).
  */
  ...
  */
  ...
  /** \details
    ...
    \param bEnable [in]  New field of view angle override state.

    \remarks
    By default field of view angle override is disabled.
  */
  void enableFovOverride(bool bEnable)
  {
    m_bFovOverride = bEnable;
  }
  ...
  */
  bool fovOverride() const
  {
    return m_bFovOverride;
  }

  /** \details
    Sets field of view override angle.

    \param fovRad [in]  Field of view override angle in radians.
  */
  ...
  */
  ...
  {
    return ((*static_cast<const OdGiBackgroundTraitsData*>(this) == static_cast<const OdGiBackgroundTraitsData&>(data2)) &&
            OdEqual(m_longRot, data2.m_longRot) && OdEqual(m_latRot, data2.m_latRot) &&
            (m_bFovOverride == data2.m_bFovOverride) && (!m_bFovOverride || OdEqual(m_fovAngle, data2.m_fovAngle)));
  }
...
/** \details
    ...
    <group OdGi_Classes>
*/
class FIRSTDLL_EXPORT OdGiCustomBackgroundTraitsData : public OdGiBackgroundTraitsData
{
  void *m_pData;
protected:
  void clear();
...
public:
  /** \details
    Default constructor for the OdGiCustomBackgroundTraitsData class. Sets the 
    OdGiDrawable::kEnvironmentBackground type for this object.
  */
  OdGiCustomBackgroundTraitsData() : m_pData(NULL)
  { m_type = OdGiDrawable::kCustomBackground; }

  /** \details
    Destructor for the OdGiCustomBackgroundTraitsData class.
  */
  ~OdGiCustomBackgroundTraitsData() { clear(); }

  /** \details
    Sets custom background property.

    \param pName [in]  Property name.
    \param pData [in]  Property data.
  */
...
inline void OdGiBackgroundTraitsData::deleteBackgroundTraitsData(OdGiBackgroundTraitsData* pBackgroundTraits)
{
  switch (pBackgroundTraits->type())
  {
    ...
    break;
    case OdGiDrawable::kEnvironmentBackground:
      delete static_cast<OdGiEnvironmentBackgroundTraitsData*>(pBackgroundTraits);
    break;
    case OdGiDrawable::kCustomBackground:
      delete static_cast<OdGiCustomBackgroundTraitsData*>(pBackgroundTraits);
    break;
    default:
      ODA_FAIL();
  }
