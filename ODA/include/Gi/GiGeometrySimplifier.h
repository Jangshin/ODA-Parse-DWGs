...
protected:
  ...
  enum SimplifierFlags
  {
    ...
    kSimplEdgeJoint        = (1 << 5), // Indexed polylineOut() call represents open joint edge.
    kSimplNoFaceNormals    = (1 << 6), // Disable normals passing in triangleOut() for non-triangle faces.
    kSimplGenTrgNormals    = (1 << 7), // Enable triangleOut() normals generation.
...
private:
  ...
  // bim specific for patterns
  unsigned int m_nAccumPatLines;
  unsigned int m_nPatLines;
...
public:
  /** \details
    Default constructor for the OdGiGeometrySimplifier class.
  */
  OdGiGeometrySimplifier();
  
  /** \details
    Destructor for the OdGiGeometrySimplifier class.
  */
  ~OdGiGeometrySimplifier();
  ...
  */
  ...
  {
    switch (m_simplFlags & (kSimplEdgeVisible | kSimplEdgeSilhouette | kSimplEdgeJoint))
    { case kSimplEdgeVisible: return kOdGiVisible; case kSimplEdgeSilhouette: return kOdGiSilhouette;
      case kSimplEdgeJoint:   return kOdGiJoint;   default: return kOdGiInvisible;
    }
  }
...
protected:
  ...
  inline void polylineOut(OdInt32 numPoints, const OdInt32* vertexIndexList, const OdGiEdgeData* pEdgeData, OdInt32 nEdgeIndex)
  {
    ...
    { const OdUInt8 edgeVisibility = pEdgeData->visibility()[nEdgeIndex];
      SETBIT(m_simplFlags, kSimplEdgeVisible,    GETBIT(edgeVisibility, kOdGiVisible));
      SETBIT(m_simplFlags, kSimplEdgeSilhouette, GETBIT(edgeVisibility, kOdGiSilhouette));
      SETBIT(m_simplFlags, kSimplEdgeJoint,      GETBIT(edgeVisibility, kOdGiJoint));
    }
    polylineOut(numPoints, vertexIndexList);
    // Restore default flags state
    SETBIT_0(m_simplFlags, kSimplEdgeSilhouette | kSimplEdgeJoint);
