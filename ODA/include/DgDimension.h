...
public:
  ...
  OdDgAssocPointRootPtr getDimAssoc() const;
  void     setDimAssoc(const OdDgAssocPointRootPtr& pAssocData);
...
private:
  ...
  OdDgAssocPointRootPtr m_pDimPointAssoc;
};
//-------------------------------------------------------------------
// OdDgDimTextFormat
//-------------------------------------------------------------------
...
public:
  ...
  virtual bool removePoint(OdUInt32 uIndex);
  ...
  virtual void setDimTextInfo( const OdDgDimTextInfo& info, bool bCheckFontId = false);
  ...
  virtual void setAnnotationScale( double dScale, bool bRescaleGeometry = true);
	...
	virtual void     setAnnotationScaleFlag(bool bSet, double dAnnotationScale = 0, bool bRescaleGeometry = true);
  ...
  virtual void setAlignmentView( OdUInt32 uView );
	...
	virtual bool getLSDDimensionFontFlag() const;
	// Sets the View number (OdUInt32) to which the PlacementAlignment applies, when set to kPaView mode.  Only used in kPaView mode by a tool which has been overridden to make use of this property.
...
public:
  ...
  // master units name for primary units.
  virtual OdString getPrimaryMasterUnitName() const;
  virtual void     setPrimaryMasterUnitName(const OdString& strUnitName);

  // sub units name for primary units.
  virtual OdString getPrimarySubUnitName() const;
  virtual void     setPrimarySubUnitName(const OdString& strUnitName);

  // master units name for secondary units.
  virtual OdString getSecondaryMasterUnitName() const;
  virtual void     setSecondaryMasterUnitName(const OdString& strUnitName);

  // sub units name for secondary units.
  virtual OdString getSecondarySubUnitName() const;
  virtual void     setSecondarySubUnitName(const OdString& strUnitName);
...
public:
  ...
  NoteTerminatorType getTerminator() const;
  void   setTerminator( NoteTerminatorType val );
...
public:
  ...
  virtual void setMultilineText(OdDgDimension* pDim, OdUInt32 uPointIndex, OdDgTextNode2dPtr& pTextNode);
  ...
  bool getUseStackOffset(const OdDgDimension* obj, const OdGePoint3d& ptStart, const OdGePoint3d& ptEnd, 
    const OdDgDimPoint& ptDim, const OdGeVector3d& vrNormal, double dDimValue, double dTextWidth,
    double dTextHeight, double dStackOffset);
...
/** \details
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgRecomputeDimPointsPE : public OdRxObject
{
public:
  ODRX_DECLARE_MEMBERS(OdDgRecomputeDimPointsPE);

  /** \details
    Calculate dimension points using association data.
  */
  virtual OdResult recomputeDimensionPoints(OdDgDimension* obj) = 0;
};
/** \details
  This template class is a specialization of the OdSmartPtr class for OdDgRecalculateDimPointsPE object pointers.
*/
typedef OdSmartPtr<OdDgRecomputeDimPointsPE> OdDgRecomputeDimPointsPEPtr;
