...
public:
  ...
  */
  ...
  /** \details
      Creates APC thread object. This method is expected for threads that are captured and
      managed on user side creation, as it guarantees access to threads for temporary operations
      managed by Thread Pool Service.
      \returns A smart pointer to newly created APC Thread object if there is no avalable APC Threads
      in the pool, or available APC Tread object from the Thread Pool Service.
  */
...
public:
  void for_each( OdApcQueue* pQueue, It first, It last, Fn fn ) {
    unsigned long n = static_cast<unsigned long>(last-first);
    if( pQueue && n>1 ) {
      m_cur = first;
      m_last = last;
      m_fn = fn;
