...
/** \details
  This class is used to store information about loop
  and to detect type of loop (inner/outer/undefined).

  Corresponding C++ library: TD_Ge

  <group !!RECORDS_tkernel_apiref>
*/
class GE_TOOLKIT_EXPORT OdGeLoopCtx
{
public:
  /** \details
  ...
  */
  ...
  /** \details
    ...
    \param pSurface [in]  Surface.
    \param arrCoedges [in]  Array of coedges, i.e. 2d curves forming the loop with reverse flags, indicating whether 2d and 3d curves have opposite orientations.
    \param arrEdges [in]  Array of edges, i.e. 3d curves forming the loop.
    \param bFaceReversed [in]  Is face reversed relative to the surface.
  */
  OdGeLoopCtx(const OdGeSurface* pSurface, const OdArray<std::pair<const OdGeCurve2d*, bool>>& arrCoedges, const OdArray<const OdGeCurve3d*>& arrEdges, bool bFaceReversed)
    ...
    , m_arrEdges(arrEdges)
    , m_bFaceReversed(bFaceReversed)
  {}

  /** \details
    Determines loop type.
    
    \param dTol [in]  Geometric tolerance.
...
private:
  const OdGeSurface* m_pSurface;
  const OdArray<std::pair<const OdGeCurve2d*, bool>>& m_arrCoedges;
  const OdArray<const OdGeCurve3d*>& m_arrEdges;
  bool m_bFaceReversed;
};
...
// Same with previous functions, but retrieves result as plane
...
/** \details
    ...
    \param tol [in]  Geometric tolerance.
    \param validateCoplanar [in]  Check whether curves/points are coplanar (set false only if curves/points are coplanar).
...
/** \details
    ...
    \param curves [in]  Array of 3D curves.
    \param reverseFlags [in]  Array of reverse flags.
    \param numCurves [in]  Number of curves.
    \param plane [out]  Receives the calculated plane.
    \param tol [in]  Geometric tolerance.
    \param validateCoplanar [in]  Check whether curves/points are coplanar (set false only if curves/points are complanar).
...
/** \details
    ...
    \param propU [out] Receives the properties for the U parameter.
    \param propV [out] Receives the properties for the V parameter.
    \param tol [in] Geometric tolerance.
...
/** \details
    ...
    \param val [in] OdGe::EntityId value.
...
/** \details
  ...
  \returns 
  true if polygon is out of the rectangle or if polygon contains a rectangle. 
