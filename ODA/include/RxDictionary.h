...
public:
  ...
  */
  ...
  inline OdSmartPtr<OdRxDictionary> subDict(const OdString* path, int numSteps);
};
...
inline OdRxDictionaryPtr OdRxDictionary::subDict(const OdString* path, int numSteps) {
  OdRxDictionaryPtr dir(this);
  for (int i = 0; !dir.isNull() && i < numSteps; ++i)
    dir = OdRxDictionary::cast(dir->getAt(path[i]));
  return dir;
}
