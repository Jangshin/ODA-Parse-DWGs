...
class TG_EXPORT OdDgRasterFrame : public OdDgGraphicsElement
{
  ...
  enum OdDgRasterFrameInitData
  {
    kSkipInit = 0,
    kInitRasterAttachmentByRasterFrameData = 1,
    kInitRasterFrameByRasterAttachmentData = 2
  };
  ...
  public:
    ...
    virtual OdDgElementIteratorPtr  createClipBoundaryElementIterator(bool atBeginning = true, bool skipDeleted = true) const;
    virtual void           addClipBoundaryElement(OdDgElementPtr& pClipElement);

    virtual OdDgElementIteratorPtr  createClipMaskElementIterator(bool atBeginning = true, bool skipDeleted = true) const;
    virtual void           addClipMaskElement(OdDgElementPtr& pClipElement);
    ...
    void          setRasterReferenceId( const OdDgElementId& idRasterReference, OdDgRasterFrameInitData initMode = kInitRasterFrameByRasterAttachmentData);
	...
	OdUInt32	  getPdfLayerCount() const;
	OdString	  getPdfLayerName(OdUInt32 uIndex) const;
	bool		  isPdfLayerVisible(const OdString& strName) const;
	void		  setPdfLayerVisibility(const OdString& strName, bool bVisibleStatus);
