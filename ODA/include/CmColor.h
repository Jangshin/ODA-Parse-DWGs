...
#include "DbExport.h"
#include "ColorBookReader/OdColorBook.h"
...
public:
  ...
  */
  ...
  /** \details
    Setup OdCmColor by color from book color represented by colorName in the book specified by bookName.

  \param color        [out] output OdCmColor.
  \param bookMap      [in] pointer to map with color books
  \param bookName     [in] book name.
  \param colorName    [in] color name.
  \returns * eOk if setup succesful, otherwise return eInvalidInput if one of input arguments is empty.
           * eKeyNotFound if book not found in input bookMap.
           * eBadColor if color not found.
           * eInvalidInput if bookMap not exists or colorName is empty.
*/
  TOOLKIT_EXPORT_STATIC static OdResult odcmGetColorFromColorBookName(OdCmColor& color, const BooksMap* bookMap,
                                                                      const OdString& bookName, const OdString& colorName);
  ...
  */
  void dxfIn(
    OdDbDxfFiler* pFiler, int groupCodeOffset, bool bFixColor);
private:
  enum NameFlags
  {    
    kNoNames      = 0,
