...
public:
  ...
  virtual void breakAssociation(const OdDgElementId& elementId);
  virtual void restoreAssociation(const OdDgElementId& elementId);
private:
...
public:
  ...
  OdGePoint3d getBottomLeftPosition() const;
  ...
  // For non-dbro tags only
  void set3dFlag( bool bSet );
...
#define TEXT_EXT_FLAG_DEF(name) \
  ...
  void  setUseAnnotationScaleFlag(bool value, double dAnnotationScale = 0, bool bRescaleGeometry = true);

  double getAnnotationScale() const;
  void   setAnnotationScale(double value, bool bRescaleGeometry = true);
  ...
  void setLinkages( OdUInt16 appId, const OdRxObjectPtrArray& linkages, bool bUpdateElementData = true);
  void removeLinkages( OdUInt16 appId, bool bUpdateElementData = true );
  void addLinkage( OdUInt16 linkageId, OdRxObjectPtr pLinkage, bool bUpdateElementData = true );
  void addLinkage( OdUInt16 linkageId, OdUInt8* pData, OdUInt32 dataSize, bool bUpdateElementData = true);
};
