...
public:
  ...
  */
  ...
  /** \details
    ...
    \param pFiler [in]  Pointer to the OdDbDwgFiler object.
    \param curve [out]  shared pointer that recieves read OdGe curve. May return null.

    \returns eOk if reading succeeds and error code otherwise.
  */
  ...
  /** \details
    ...
    \param pFiler [in]  Pointer to the OdDbDwgFiler object.
    \param curve [in] shared pointer to OdGe curve to be written.

    \remarks
    When curve pointer is not null then this function writes boolean value true indicating
    that curve data is follows, then writes curve type code as 32-bit integer and then
    writes curve data calling corresponding outFields() method. When curve pointer is null
    then this function writes only boolean value false indicating that curve is absent.
  */
