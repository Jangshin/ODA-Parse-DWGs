...
namespace OdDb
{
  ...
  */
  ...
  {
    /** Disables creating of default objects in every drawing. */
    kEnabler = 0,
    /** Enables creating of default objects in every drawing. */
    kEditor = 1
  };
...
public:
  ...
  */
  ...
  /** \details 
    Incrementally saves the database contents to the specified output stream buffer. 
  ...
  */
  ...
  /** \details 
    Incrementally saves the database contents to the specified output .dwg file.
  ...
  */
  ...
  /** \details
    ...
    This function is used to clone objects within a single database. To clone objects to a different database, use OdDbDatabase::wblockCloneObjects().
  ...
  */
  ...
  /** \details
    Clones a set of objects from this database to another one (target database) and appends the clones to the specified owner object.

    \param objectIds    [in]  Array of object IDs of the objects to be cloned.
    \param ownerId      [in]  Object ID of the owner object (should belong to the target database).
    \param idMap        [in/out] ID map.
    \param deferXlation [in] Defer translation.
    ...
    This function is used to clone objects between databases. To clone within a single database, use OdDbDatabase::deepCloneObjects().
    ...
    The sequence of reactors called by OdDbDatabase::wblockCloneObjects:
    - OdRxEventReactor::beginDeepClone()
    - OdRxEventReactor::beginWblockObjects()
    - OdRxEventReactor::beginDeepCloneXlation() (if deferXlation is false)
    - OdRxEventReactor::endDeepClone() (if deferXlation is false)
    - OdRxEventReactor::abortDeepClone() (if cloning failed/was aborted)
...
//DOM-IGNORE-END
  ...
  */
  ...
  /** \details
    ...
    \remarks
    CGEOCS represents the name of the GIS coordinate system assigned to the drawing file.
  */
  ...
  */
  ...
  /** \details
    ...
    * GroupDictionary
    ...
    Transformation matrix should be uniform. Non-uniform transform is ignored.
  */
  void insert(
  ...
  */
  ...
  /** \details
    Clones specified objects of this database to the model space of the database pointed to by pOutputDb

    \param pOutputDb [in]  Existing database.
    \param outObjIds [in]  Array of object IDs to be cloned.
    \param basePoint [in]  WCS base point for the output database (INSBASE).
    \param drc [in]  must be one of the following:

    <table>
    Name                        Description
    OdDb::kDrcIgnore            If a duplicate record exists, use the existing record in the database and ignore the clone.
    OdDb::kDrcReplace           If a duplicate record exists, replace it with the cloned record.
    OdDb::kDrcMangleName        Incoming record names are mangled with $0$<name>.
    OdDb::kDrcUnmangleName      Unmangle the names mangled by OdDb::kDrcMangleName, then default to OdDb::kDrcIgnore. Typically used by RefEdit when checking records into the original database.
    </table>

    \remarks
    The method expects the output database to be initialized (either read from file, or created with default objects).
    All objects are copied with their dependencies. For example, if a text entity uses a specific textstyle, it will be copied as well; line entities will be copied with their linestyles, etc.
  */
  void wblock(OdDbDatabase* pOutputDb, const OdDbObjectIdArray& outObjIds, const OdGePoint3d& basePoint, OdDb::DuplicateRecordCloning drc);
...
/** \details
    ...
    \param pDb [in] OdDbDatabase object.
    \param bValue [in] New state of the reactor suppression.
    \returns Old state of the reactor suppression.
*/
