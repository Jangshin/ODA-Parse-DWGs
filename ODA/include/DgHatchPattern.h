...
public:
  ...
  virtual double getHatchScale() const = 0;
  virtual void   setHatchScale(double dScale) = 0;

  // flags
  virtual bool getSnappableFlag() const = 0;
  virtual void setSnappableFlag( bool value ) = 0;

  virtual bool getUseAnnotationScaleFlag() const = 0;
  virtual void setUseAnnotationScaleFlag(bool value) = 0;
...
struct TG_EXPORT DWGHatchLine
{
  ...
  {
    m_throughPoint *= dScale;
    m_offset *= dScale;

    for (OdUInt32 i = 0; i < m_dashes.size(); i++)
      m_dashes[i] *= dScale;
  }
};
