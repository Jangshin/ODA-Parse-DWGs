...
*/
#define DGELEMENT_CONSTR(ClassName) OdSmartPtr<ClassName> (new ClassName, kOdRxObjAttach)
...
{ 
  ...
  */
  enum DuplicateRecordCloning
  {
	...
	// change drawing order mode
	enum OdDgChangeDrawingOrderMode
	{
		kSendToBack		= 0,
		kMoveForwards  = 1,
		kMoveBackwards = 2,
...
public:
  ...
  virtual OdResult getGeomExtents(OdGeExtents3d& extents) const; //block overloading
  virtual OdResult getGeomExtents(const OdDgElementId& idView, OdUInt32 uExtentFlags, OdGeExtents3d& extents) const;
  virtual OdResult getGeomExtents(OdUInt32 uExtentFlags, OdGeExtents3d& extents) const;
  ...
  */
  virtual bool getLastModifiedTime(OdTimeStamp& uRetTime) const;

  /** \details
  Set the time of the last modification.
  */
  virtual bool setLastModifiedTime(const OdTimeStamp& uTime);
...
*/
  ...
  */
  ...
  /** \details
    If true then composeForLoad will be called second time in single thread mode after multithread loading.
  */
  virtual bool needPostprocessingAfterMTLoading();

  /** \details
    After loading from file, performs necessary actions which require internal initialization or 
    accessing other Database objects.

    \param bMTMode [in]  Function is called in multithread mode.
    \param bPartialLoading [in]  Function is called after partial loading.
  */

  virtual void composeForLoad(bool bMTMode, bool bPartialLoading);

  /** \details
  Determines the behavior for custom objects when saving.
*/
  ...
  */
  ...
  /** \details
   ...
   This function is called whenever the geometry of element as been modified. It therefore
   indicates only that the graphics for it may have changed.
 */
  virtual void modifiedGeometry(
    const OdDgElement* pElement);
  ...
  */
  ...
  virtual OdResult explodeGeometry( OdRxObjectPtrArray& entitySet, double dDeviation = 0 ) const;
  ...
  */
  ...
  virtual OdResult explodeGeometryToBlock( OdDgElement* pParentElement, OdDgElementIdArray *ids = 0, double dDeviation = 0);
 ...
 */
  ...
  virtual bool getGeometryModified() const;
...
/** \details
    ...
    <group OdDg_Classes>
*/
class TG_EXPORT OdDgElementXDataPE : public OdRxObject
{
public:
  ODRX_DECLARE_MEMBERS(OdDgElementXDataPE);

  /** \details
   Returns the XData Linkage value.
 */
  virtual OdRxObjectPtr getXDataLinkage( const OdDgElement* pElm, const OdString& appName = OdString::kEmpty) const = 0;

  /** \details
    Sets the XData Linkage value.
  */
  virtual void setXDataLinkage( OdDgElement* pElm, OdRxObjectPtr pResBuf) = 0;
 ...
 */
  ...
  virtual void copyLinkageReferencesForWBlockClone( const OdDgElement* pElm, OdDgFiler* pFiler) const = 0;
  ...
  */
  ...
  virtual void updateLinkageReferencesForWBlockClone( OdDgElement* pElm, const OdDgIdMapping& idMap) = 0;
};

/** \details
  This template class is a specialization of the OdSmartPtr class for OdDgElementXData object pointers.
*/

typedef OdSmartPtr<OdDgElementXDataPE> OdDgElementXDataPEPtr;
