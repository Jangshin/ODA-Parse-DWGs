...
class OdDgGeoDataReprojectionSettings;
class OdDgGeoDataReprojectionCoordinateTransformer;
...
public:
  ...
  enum OdDgGeoDataCoordinateSystemChangeAction
  {
    kNoAction              = 0,
    kChangeStorageUnits    = 1,
    kChangeMasterUnits     = 2,
    kReprojectCoordSystem  = 3
  };
  ...
  struct UnitDescription
  {
    ...
    UnitDescription(const UnitDescription& obj)
    {
      m_base = obj.m_base;
    ...
    UnitDescription& operator=(const UnitDescription& obj)
    {
      m_base = obj.m_base;
      m_system = obj.m_system;
      m_numerator = obj.m_numerator;
  ...
  enum OdDgUnitsAccuracy
  {
    kAccuracyNone           = 0,  // 0.0
    kAccuracyDecimal1       = 1,  // 0.1
    kAccuracyDecimal2       = 2,  // 0.12
    kAccuracyDecimal3       = 3,  // 0.123
    kAccuracyDecimal4       = 4,  // 0.1234
    kAccuracyDecimal5       = 5,  // 0.12345
    kAccuracyDecimal6       = 6,  // 0.123456
    kAccuracyDecimal7       = 7,  // 0.1234567
    kAccuracyDecimal8       = 8,  // 0.12345678
    kAccuracyFractional2    = 9,  // 1/2
    kAccuracyFractional4    = 10, // 1/4
    kAccuracyFractional8    = 11, // 1/8
    kAccuracyFractional16   = 12, // 1/16
    kAccuracyFractional32   = 13, // 1/32
    kAccuracyFractional64   = 14, // 1/64
    kAccuracyFractional128  = 15, // 1/128
    kAccuracyFractional256  = 16, // 1/256
    kAccuracyExponential1   = 17, // 1.1E+01
    kAccuracyExponential2   = 18, // 1.12E+01
    kAccuracyExponential3   = 19, // 1.123E+01
    kAccuracyExponential4   = 20, // 1.1234E+01
    kAccuracyExponential5   = 21, // 1.12345E+01
    kAccuracyExponential6   = 22, // 1.123456E+01
    kAccuracyExponential7   = 23, // 1.1234567E+01
    kAccuracyExponential8   = 24, // 1.12345678E+01
  };

  enum OdDgAngleDirectionMode
  {
    kAzimut   = 1,
    kBearing  = 2
  };

  enum OdDgAngleAccuracy
  {
      kAngleAccuracy0 = 0x00      //DECIMAL_0                = 0x0001,
    , kAngleAccuracy1 = 0x01      //DECIMAL_01               = 0x0002,
    , kAngleAccuracy2 = 0x02      //DECIMAL_012              = 0x0003,
    , kAngleAccuracy3 = 0x03      //DECIMAL_0123             = 0x0004,
    , kAngleAccuracy4 = 0x04      //DECIMAL_01234            = 0x0000,
    , kAngleAccuracy5 = 0x05      //DECIMAL_012345           = 0x0006,
    , kAngleAccuracy6 = 0x06      //DECIMAL_0123456          = 0x0007,
    , kAngleAccuracy7 = 0x07      //DECIMAL_01234567         = 0x0008,
    , kAngleAccuracy8 = 0x08      //DECIMAL_012345678        = 0x0009,
  };
  ...
  */
  ...
  virtual OdResult subGetGeomExtents(OdGeExtents3d& extent) const;
  virtual OdResult subGetGeomExtents(const OdDgElementId& idView, OdGeExtents3d& extents) const;
  virtual OdResult subGetGeomExtents(OdUInt32 uExtentFlags, OdGeExtents3d& extent ) const;
  virtual OdResult subGetGeomExtents(const OdDgElementId& idView, OdUInt32 uExtentFlags, OdGeExtents3d& extents) const;
  ...
  OdDgUnitsFormat getWorkingUnitFormat() const;
  void setWorkingUnitFormat(const OdDgUnitsFormat& uFormat);

  // Unit accuracy

  OdDgUnitsAccuracy getWorkingUnitAccuracy() const;
  void setWorkingUnitAccuracy(const OdDgUnitsAccuracy& uAccuracy);

  // Angle format

  OdDgAngleFormat getAngleFormat() const;
  void setAngleFormat(const OdDgAngleFormat& uFormat);

  // Angle accuracy

  OdDgAngleAccuracy getAngleAccuracy() const;
  void setAngleAccuracy(const OdDgAngleAccuracy& uAccuracy);

  // Angle direction mode

  OdDgAngleDirectionMode getAngleDirectionMode() const;
  void setAngleDirectionMode(const OdDgAngleDirectionMode& uMode);

  // Angle direction clockwise flag

  bool getAngleDirectionClockwiseFlag() const;
  ...
  void     setGridReference( OdUInt32 uGridReference );
  ...
  virtual OdDgGeoDataInfoPtr getGeoData(OdDg::OpenMode uMode = OdDg::kForRead) const;
  virtual void               setGeoData(OdDgGeoDataInfo* pGeoDataInfo, const OdDgGeoDataReprojectionSettings& reprojectionSettings, OdDgGeoDataCoordinateSystemChangeAction uAction = kNoAction);
...
public:
  ...
  OdResult subGetGeomExtents(OdUInt32 uExtentFlags, OdGeExtents3d& extent ) const;
...
public:
  ...
  */
  ...
  /** \details
  Move model to the start position of the table.

  \param idModel [in]  model id to move.
  \param bSkipDeleted  [in]  skip erased elements.

  \returns
  eOk if model was moved to the start position of the table.

  \remarks
  The model must be from the table. The function is only change position
  of the model and doesn't add it to the table.
  */
  virtual OdResult moveToStart(OdDgElementId& idModel, bool bSkipDeleted = true);

  /** \details
  Move model to the end position of the table.

  \param idModel       [in]  model id to move.
  \param bSkipDeleted  [in]  skip erased elements.

  \returns
  eOk if model was moved to the end position of the table.

  \remarks
  The model must be from the table. The function is only change position
  of the table record and doesn't add it to the table.
  */
  virtual OdResult moveToEnd(OdDgElementId& idModel, bool bSkipDeleted = true);

  /** \details
  Move model to the position with defined index of the table.

  \param uIndex [in]  number from the start position into the table.
  \param idModel [in]  model id of the table record to move.
  \param bSkipDeleted  [in]  skip erased elements.

  \returns
  eOk if model was moved to the index position of the table.

  \remarks
  The model must be from the table. The function is only change position
  of the table record and doesn't add it to the table.
  */
  virtual OdResult moveToIndex(OdUInt32 uIndex, OdDgElementId& idModel, bool bSkipDeleted = true);
