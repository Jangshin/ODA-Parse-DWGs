...
public:
  ...
  */
  ...
  /** \details
    Gets current detail view style to use as default for initialization
    of new detail symbol.

    \param db [in]  database context.
    \param init [in]  Optional. If true then new style object may be created
    in the database if not exists.

    \returns
    Object ID of the current OdDbDetailViewStyle object
    that defines the appearence of this view symbol.

    \remarks
    This method checks if CVIEWDETAILSTYLE variable is initialized and checks
    if default OdDbDetailViewStyle object exists in detail-view-style dictioinary.
    When init parameter is true, method may create new object in the database
    and initialize CVIEWDETAILSTYLE variable if necessary.
  */
  static OdDbObjectId defaultDetailViewStyle(OdDbDatabase* db, bool init = true);
...
//  virtual OdDbObjectPtr subDeepClone(OdDbIdMapping& idMap, OdDbObject* pOwner, bool isPrimary = true) const;
