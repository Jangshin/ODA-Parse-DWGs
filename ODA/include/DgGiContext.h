...
/** 
  <group OdDg_Classes>
*/
class OdDgChangeLineStyleScaleModelReactor : public OdDgModelReactor
{
public:
  static OdDgChangeLineStyleScaleModelReactor* attach(const OdDgElementId& idModel, OdGiContextForDgDatabase* pContext);
  static void                                  detach(const OdDgElementId& idModel, const OdGiContextForDgDatabase* pContext);

  virtual OdGiContextForDgDatabase* getContext() const;
  virtual void  setContext(OdGiContextForDgDatabase* pContext);

  virtual void  giContextLinkedDataModified(const OdDgElement* pModel);

private:
  OdGiContextForDgDatabase* m_pContext;
};

//======================================================================================================================

class OdDgChangeLineStyleScaleDatabaseReactor;
typedef OdSmartPtr< OdDgChangeLineStyleScaleDatabaseReactor> OdDgChangeLineStyleScaleDatabaseReactorPtr;
...
/** 
  <group OdDg_Classes>
*/
class OdDgChangeLineStyleScaleDatabaseReactor : public OdDgDatabaseReactor
{
public:
  static OdDgChangeLineStyleScaleDatabaseReactorPtr attach(OdDgDatabase* pDb, OdGiContextForDgDatabase* pContext);
  static void                                       detach(OdDgDatabase* pDb, const OdGiContextForDgDatabase* pContext);

  virtual OdGiContextForDgDatabase* getContext() const;
  virtual void  setContext(OdGiContextForDgDatabase* pContext);

  virtual void  modified(const OdDgDatabase* pDb);
...
public:
  ...
  virtual void setCommonLinetypeScale( double dScale );
  ...
  virtual OdDbStub* getStubByMatName( OdDbBaseDatabase *pBaseDb, const OdString&  ) const;
  ...
  */
  ...
  void setDatabase( OdDgDatabasePtr dgnDatabase, bool bSetDatabaseReactor = true);
  ...
  bool getAllowToCreateViewportsForXRefsFlag() const;
  void setAllowToCreateViewportsForxRefsFlag(bool bSet);
  ...
  */
  virtual bool getTrueColor( OdUInt32 colorIndex, OdDgCmEntityColor& trueColor, OdDgLevelTableRecordPtr level,
                             const OdGiAuxDataForDgDatabasePtr& pAuxData, bool bAllowForLevelOverrides = true, 
                             const OdDgDatabase* pDb = NULL ) const;
  virtual bool getWeight( OdUInt32 weight, OdUInt32& resultWeight, OdDgLevelTableRecordPtr level, 
                          const OdGiAuxDataForDgDatabasePtr& pAuxData, bool bCheckViewFlag = true, bool bAllowForLevelOverrides = true) const;
  virtual bool getStyleEntryId( OdUInt32 styleEntryId, OdUInt32& resultStyleEntryId, OdDgLevelTableRecordPtr level, 
                          const OdGiAuxDataForDgDatabasePtr& pAuxData, bool bAllowForLevelOverrides = true ) const;
...
private:
  ...
  public:
    ...
    void setDatabase(OdDgDatabase * pDb, bool bSetDatabaseReactor = true);
  ...
  double                m_dCommonLineStyleScale;
  OdDgChangeLineStyleScaleDatabaseReactorPtr m_pChangeLineStyleScaleDbReactor;
  bool                  m_bBindedToView;
  bool                  m_bAllowToCreateViewportsForXRefs;
};
