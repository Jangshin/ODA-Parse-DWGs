...
public:
  ...
  */
  ...
  /** \details
    ...
    * |taperAngle| < OdaPI2 - 1e-6
    ...
    The ModelerGeometry implementation returns eNotImplementedYet if taperAngle != 0
    and the profile contains a spline curve.
  */
  ...
  */
  ...
  /** \details
    ...
    then this solid is left unchanged, pNegHalfSolid is set to NULL, and eOk is
    returned.
  ...
  */
  ...
  /** \details
    ...
    returns eInvalidInput. If the specified surface does not intersect this solid, then this solid and pNegHalfSolid are
    left unchanged, eNotApplicable is returned.
  ...
  */
  ...
  /** \details
    ...
    object reference.
    The ModelerGeometry implementation returns eNotImplementedYet if the profile contains a spline curve
    and draftAngle != 0 or endDraftDist != 0.
  */
  ...
  */
  ...
  /** \details
    Represents geometry type of a OdDb3dSolid object.
  */
  enum GeomType
  {
    /** Undefined type.*/
  ...
  /** \details
    Determines the geometry type of the OdDb3dSolid object by analyzing its history and, optionally, its BRep data.

    \param byBrep  [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param params [out] Receives parameters of the geometry.

    \returns geometry type as a GeomType value.

    \remarks
    The calling application is responsible for the memory allocated for the OdDb3dSolidGeomParams object.
    Split BRep-s (e.g., two half-spheres) are not recognized.
    BRep analyze is not implemented for extrusion, sweep, loft and revolve.
  */
  GeomType determineGeomType(bool byBrep = false, OdDb3dSolidGeomParams** params = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a sphere by analyzing its history and, optionally, its BRep data.

    \param byBrep  [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param radius [out] Receives the radius of the sphere.
    \param matrix [out] Receives the transformation matrix of the sphere.

    \returns The true value if OdDb3dSolid is a sphere, false otherwise.

    \remarks Split BRep-s (e.g., two half-spheres) are not recognized.
  */
  bool isSphere(bool byBrep = false, double* radius = nullptr, OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a torus by analyzing its history and, optionally, its BRep data.

    \param byBrep       [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param majorRadius [out] Receives the major radius of the torus.
    \param minorRadius [out] Receives the minor radius of the torus.
    \param matrix      [out] Receives the transformation matrix of the torus.

    \returns The true value if OdDb3dSolid is a torus, false otherwise.

    \remarks Split BRep-s (e.g., two half-tori) are not recognized.
  */
  bool isTorus(bool byBrep = false, double* majorRadius = nullptr, double* minorRadius = nullptr, OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a cylinder by analyzing its history and, optionally, its BRep data.

    \param byBrep   [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param height  [out] Receives the height of the cylinder.
    \param xRadius [out] Receives the major radius of the cylinder.
    \param yRadius [out] Receives the minor radius of the cylinder.
    \param matrix  [out] Receives the transformation matrix of the cylinder.

    \returns The true value if OdDb3dSolid is a cylinder, false otherwise.

    \remarks Split BRep-s (e.g., two half-cylinders) are not recognized.
  */
  bool isCylinder(bool byBrep = false,
    double* height = nullptr,
    double* majorRadius = nullptr,
    double* minorRadius = nullptr,
    OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a cone by analyzing its history and, optionally, its BRep data.

    \param byBrep      [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param height     [out] Receives the height of the cone.
    \param xRadius    [out] Receives the major radius of the cone.
    \param yRadius    [out] Receives the minor radius of the cone.
    \param topXRadius [out] Receives the major radius of the top of the cone (zero if not truncated).
    \param matrix     [out] Receives the transformation matrix of the cone.

    \returns The true value if OdDb3dSolid is a cone, false otherwise.

    \remarks Split BRep-s (e.g., two half-cones) are not recognized.
  */
  bool isCone(bool byBrep = false,
    double* height = nullptr,
    double* majorRadius = nullptr,
    double* minorRadius = nullptr,
    double* topMajorRadius = nullptr,
    OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a box by analyzing its history and, optionally, its BRep data.

    \param byBrep  [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param xLen   [out] Receives the length of the box along its X-axis.
    \param yLen   [out] Receives the length of the box along its Y-axis.
    \param zLen   [out] Receives the length of the box along its Z-axis.
    \param matrix [out] Receives the transformation matrix of the box.

    \returns The true value if OdDb3dSolid is a box, false otherwise.

    \remarks Split BRep-s are not recognized.
  */
  ...
  /** \details
    Determines whether the OdDb3dSolid object is a wedge by analyzing its history and, optionally, its BRep data.

    \param byBrep  [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param xLen   [out] Receives the dimension of the wedge along its X-axis.
    \param yLen   [out] Receives the dimension of the wedge along its Y-axis.
    \param zLen   [out] Receives the dimension of the wedge along its Z-axis.
    \param matrix [out] Receives the transformation matrix of the wedge.

    \returns The true value if OdDb3dSolid is a wedge, false otherwise.
  ...
  */
  ...
  /** \details
    Determines whether the OdDb3dSolid object is a pyramid by analyzing its history and, optionally, its BRep data.

    \param byBrep     [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param height    [out] Receives the height of the pyramid.
    \param sides     [out] Receives the quantity of sides of the pyramid.
    \param radius    [out] Receives the radius of the circle inscribed into the bottom polygon.
    \param topRadius [out] Receives the radius of the circle inscribed into the top polygon.
    \param matrix    [out] Receives the transformation matrix of the pyramid.

    \returns The true value if OdDb3dSolid is a pyramid, false otherwise.

    \remarks Split BRep-s are not recognized.
  */
  bool isPyramid(bool byBrep = false,
    double* height = nullptr,
    int* sides = nullptr,
    double* radius = nullptr,
    double* topRadius = nullptr,
    OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is an extrusion by analyzing its history and, optionally, its BRep data.

    \param byBrep           [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param direction       [out] Receives the direction of the extrusion (its length is the extrusion height).
    \param extrusionEntity [out] Receives the extrusion entity of the extrusion.
    \param options         [out] Receives the options of the extruded solid.
    \param matrix          [out] Receives the transformation matrix of the extrusion.

    \returns The true value if OdDb3dSolid is an extrusion, false otherwise.

    \remarks BRep data analyze is not implemented.
  */
  bool isExtrusion(bool byBrep = false,
    OdGeVector3d* direction = nullptr,
    OdDbEntity** extrusionEntity = nullptr,
    OdDbSweepOptions* options = nullptr,
    OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a swept solid by analyzing its history and, optionally, its BRep data.

    \param byBrep       [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param sweepEntity [out] Receives the sweep entity of the swept solid.
    \param pathEntity  [out] Receives the path entity of the swept solid.
    \param options     [out] Receives the options of the swept solid.
    \param matrix      [out] Receives the transformation matrix of the swept solid.

    \returns The true value if OdDb3dSolid is a swept solid, false otherwise.

    \remarks BRep data analyze is not implemented.
  */
  bool isSweep(bool byBrep = false,
  ...
  /** \details
    Determines whether the OdDb3dSolid object is a lofted solid by analyzing its history and, optionally, its BRep data.

    \param byBrep              [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param crossSectionCurves [out] Receives the array of the cross section curves of the lofted solid.
    \param guideCurves        [out] Receives the array of the guide curves of the lofted solid.
    \param pathCurve          [out] Receives the path curve of the lofted solid.
    \param options            [out] Receives the options of the lofted solid.
    \param matrix             [out] Receives the transformation matrix of the lofted solid.

    \returns The true value if OdDb3dSolid is a lofted solid, false otherwise.

    \remarks BRep data analyze is not implemented.
  */
  bool isLoft(bool byBrep = false,
    OdDbEntityPtrArray* crossSectionCurves = nullptr,
    OdDbEntityPtrArray* guideCurves = nullptr,
    OdDbEntity** pathCurve = nullptr,
    OdDbLoftOptions* options = nullptr,
    OdGeMatrix3d* matrix = nullptr) const;

  /** \details
    Determines whether the OdDb3dSolid object is a revolved solid by analyzing its history and, optionally, its BRep data.

    \param byBrep         [in] Flag whether to analyze BRep data if history is absent or doesn't specify geometry type.
    \param revolveAngle  [out] Receives the revolve angle of the revolved solid.
    \param startAngle    [out] Receives the start angle of the revolved solid.
    \param axisPoint     [out] Receives the point on the revolved solid's axis.
    \param axisDir       [out] Receives the direction of the revolved solid's axis.
    \param revolveEntity [out] Receives the revolve entity for the revolved solid.
    \param options       [out] Receives the options of the revolved solid.
    \param matrix        [out] Receives the transformation matrix of the revolved solid.

    \returns The true value if OdDb3dSolid is a revolved solid, false otherwise.

    \remarks BRep data analyze is not implemented.
  */
  bool isRevolve(bool byBrep = false,
    double* revolveAngle = nullptr,
    double* startAngle = nullptr,
    OdGePoint3d* axisPoint = nullptr,
    OdGeVector3d* axisDir = nullptr,
    OdDbEntity** revolveEntity = nullptr,
    OdDbRevolveOptions* options = nullptr,
    OdGeMatrix3d* matrix = nullptr) const;
