...
void OdExGiRasterImage::setModule(const OdRxModulePtr& pModule)
{
  m_pModule = pModule;
}
...
void OdExGiRasterImage::getColorAt(OdUInt32 x, OdUInt32 y, OdUInt8& blue, OdUInt8& green, OdUInt8& red) const
{
  ...
  const OdUInt8* ptr = pIm + size_t(y) * lineSize + x * m_header.m_bitPerPixel / 8;
...
void OdExGiRasterImage::copyFrom(const OdRxObject* pOtherObj)
{
  ...
  m_transparentColor = pFrom->m_transparentColor;
  m_pModule          = pFrom->m_pModule;
}
...
int OdExGiRasterImage::transparentColor() const
{
...
void OdExGiRasterImage::scanLines(OdUInt8* pBytes, OdUInt32 index, OdUInt32 numLines) const
{
  ...
  memcpy(pBytes, m_bits.asArrayPtr() + size_t(index)*scLSz, numLines*scLSz);
}
    ...
    case 2L: // RLE4
      {
        ...
        for (bool bContinue = true; bContinue && !pBuf->isEof();)
        {
                ...
                default:
                  {
                    if (scanline >= int(uHeight))
                      return; // End of file reached
                    OdUInt8 *sline = pData + (scanline * scanLineSize); // Get row
                    if (OdUInt32(bits) <= uWidth)
                    {
                      const OdUInt32 bytes2read = (OdUInt32)odmin(int(status_byte), uWidth - bits);
                      pBuf->getBytes(sline + bits, bytes2read);
                      if (bytes2read < status_byte)
                        pBuf->seek(status_byte - bytes2read, OdDb::kSeekFromCurrent);
                    } else
                      pBuf->seek(status_byte, OdDb::kSeekFromCurrent);
                    if ((status_byte & 1) == 1)
                      second_byte = pBuf->getByte();
                    bits += status_byte;
                  }
...
{
  const BITMAPINFOHEADER *pBmpInfo = fiImage.getInfoHeader();
  ...
  if (fiImage.getPaletteSize())
  {
    ...
    // Note: SetTransparentIndex/GetTransparentIndex functions doesn't wrapped by fipImage class.
    bitmap.setTransparentColor(::FreeImage_GetTransparentIndex(*(fipImage*)OdIntToPtr(&fiImage)));
  }
  const OdUInt64 memSize = OdUInt64(fiImage.getScanWidth()) * pBmpInfo->biHeight;
  if (memSize > std::numeric_limits<size_t>::max())
    throw OdError(eOutOfMemory);
  bitmap.bits().resize(size_t(memSize));
...
OdGiRasterImagePtr ExRasterModule::loadRasterImage(OdStreamBuf *pBuf, const OdUInt32 *pFlagsChain)
{
  ...
  if(scanLinesPos!=0) // if zero -- there is no palette (cr1475, GU)
  {
    ...
    if (   colDepth <= 8
        || compr == 3L) // == BI_BITFIELDS // keep palette for OdLoadBitFieldsBMPData // CORE-19797
    {
    ...
    { // #CORE-19463 : Non-indexed image don't require palette, so we can skip palette block which may be written by mistake.
      bitmap.palette().data().resize(0);
      if (paletteSize)
        pBuf->seek(paletteSize, OdDb::kSeekFromCurrent);
    }
  }
  ...
  {
    // no compression
    try { // AutoCAD never mind about end of file, it doesn't fill pixels by anything in this case.
      if (!bUpsideDown)
      {
        OdUInt32 len = static_cast<OdUInt32>(pBuf->length() - pBuf->tell());
        if (scanLinesSize > OdUInt32(len) && (scanLinesSize - OdUInt32(len)) <= 8)
          scanLinesSize = len; // CORE-17010 is not critical (photoshop reads such files)
...
static void fiImageFromGiRasterImage(const OdGiRasterImage *pRaster, fipImage &fiImage, FREE_IMAGE_FORMAT fiFormat, const OdUInt32 *pFlagsChain)
{
  ...
  OdGiRasterImage::Units units = pRaster->defaultResolution(xResolution, yResolution);
  if (units != OdGiRasterImage::kNone && xResolution > 0. && yResolution > 0.)
  {  // Note: functions below require resolution in dots/cm
    double dCentimetersInUnit = OdGiRasterImage::millimetersInUnit(units) / 10.;
    fiImage.setHorizontalResolution(xResolution / dCentimetersInUnit);
    fiImage.setVerticalResolution(yResolution / dCentimetersInUnit);
  }
  ...
  if (fiImage.getPaletteSize() && pRaster->paletteDataSize())
  {
    ...
    if (pRaster->transparentColor() >= 0)
      // Note: SetTransparentIndex/GetTransparentIndex functions doesn't wrapped by fipImage class.
      ::FreeImage_SetTransparentIndex(fiImage, pRaster->transparentColor());
  }
...
struct odStreamFIAdapter : public FreeImageIO
{
  ...
  {
    odStreamFIAdapter *pAdapter = static_cast<odStreamFIAdapter*>(handle);
    ...
    if (diff + nBytesToRead > pAdapter->m_pStreamBuf->length())
      nBytesToRead = static_cast<OdUInt32>(pAdapter->m_pStreamBuf->length() - diff);
    if (nBytesToRead)
                                        ...
                                        OdStreamBuf* pStreamBuf, const OdUInt32 *pFlagsChain)
{
  ...
  switch( type )
  {
    ...
    case OdRxRasterServices::kJPEG:
      ...
      break;
    case (OdUInt32)OdRxRasterServices::kUnknown:
      break;
  }
#else // !defined(RASTER_FREE_IMAGE)
  int fiFlags;
