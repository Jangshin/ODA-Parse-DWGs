...
public:
  //ODRX_DECLARE_MEMBERS(OdBaseFileBuf);
 ...
 /** \details
  Returns the access mode for this file object.
*/
 virtual OdUInt32 getAccessMode() const;
...
public:
  //ODRX_DECLARE_MEMBERS(OdRdFileBuf);
...
#else
  OdUInt8 m_pBuffer[WRITING_BUFFER_LENGTH + 1] = { 0 }; // +1 is to suppress /analyse false positive
#endif
...
public:
  //ODRX_DECLARE_MEMBERS(OdWrFileBuf);
  ...
  /** \details
    ...
    Opens filename if specified.
  */
  OdWrFileBuf(const OdString& filename):m_nBufferedSize(0) 
  { 
#if defined(_MSC_VER) && (_MSC_VER < 1900)
...
{
public:
  OdBaseFileBuf(const OdBaseFileBuf&) = delete;
  OdBaseFileBuf& operator = (const OdBaseFileBuf&) = delete;
  //ODRX_DECLARE_MEMBERS(OdBaseFileBuf);
...
{
public:
  OdWrFileBuf(const OdWrFileBuf& source) = delete;
  OdWrFileBuf& operator = (const OdWrFileBuf& source) = delete;
  //ODRX_DECLARE_MEMBERS(OdWrFileBuf);
