...
template <class T> class OdMemoryAllocator64
{ public:
    typedef size_t size_type;
    static inline void copyAssignRangeDisjoint(T *pDestination, const T *pSource, size_type numElements)
    { memcpy(pDestination, pSource, numElements * sizeof(T)); }
    static inline void copyAssignRange(T *pDestination, const T *pSource, size_type numElements)
    { memmove(pDestination, pSource, numElements * sizeof(T)); }
    static inline void moveAssignRange(T *pDestination, T *pSource, size_type numElements)
    { memmove(pDestination, pSource, numElements * sizeof(T)); }
    ...
    { *pElement = value; }
    static inline void copyConstructFill(T *pDestination, size_type numElements, const T &value)
    { while (numElements--) { pDestination[numElements] = value; } }
    static inline void copyConstructRange(T *pDestination, const T *pSource, size_type numElements)
    { copyAssignRangeDisjoint(pDestination, pSource, numElements); }
    static inline void moveConstructRange(T *pDestination, T *pSource, size_type numElements)
    { copyAssignRangeDisjoint(pDestination, pSource, numElements); }
    static inline void defaultConstructFill(T *pDestination, size_type numElements)
    { /* DOES NOTHING */ }
    static inline void destroy(T *pObject)
    { /* DOES NOTHING */ }
    static inline void destroyRange(T *objects, size_type numObjects)
    { /* DOES NOTHING */ }
    static inline void rotate(T *first, T *middle, T *last)
    { const T tmp(*middle); memmove(first + 1, first, (last - first) * sizeof(T)); *first = tmp; }
    static inline bool useRealloc()
    { return true; }
};
typedef OdVector<OdUInt8, OdMemoryAllocator64<OdUInt8>> OdRasterData;
...
public:
  ...
  public:
    ...
    OdUInt32 numColors() const { return (m_colorsData.size() + 3) / 4; }
  ...
  public:
    ...
    */
    ...
    void setModule(const OdRxModulePtr& pModule);
    ...
    */
    ...
    {
      return size_t(((xPixels * bitsPerPixel +31) & ~31) /8) * yPixels;
    }
    ...
    */
    void setBits(const OdRasterData& data)
    {
      ODA_ASSERT(data.size() <= bitDataSize());
      m_bits = data;
    }
    
    void setBits(const OdUInt8* data, size_t numBytes)
    {
