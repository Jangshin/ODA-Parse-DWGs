  ...
  : m_CodePageId(CP_UNDEFINED)
{
  ...
  {
    m_CodePageId = CP_UTF_8;
  }
 #endif
#elif  (defined(__linux__) || defined(EMCC))  && !defined(ANDROID)
  m_CodePageId = CP_UTF_8;
#endif
}
    ...
    Oda::FileCreationDisposition dispos)
{
  ...
  if (access == (Oda::kFileRead | Oda::kFileWrite))
    m_CodePageId = CP_UNDEFINED;
#endif
...
inline bool isRxFSPath(const TChar* s) { if (*s == '\0') return false; return (s[2]==':' && s[0]=='r' && s[1]=='x'); }
...
OdResult RxSystemServicesImpl::getEnvVar(const OdString &varName, OdString &value)
{
  ...
  {
    {
      ...
      pszOldVal = (LPTSTR)malloc(buffSize * sizeof(TCHAR));
      if (NULL == pszOldVal)
      {
        return eOutOfMemory;
      }

      dwRet = GetEnvironmentVariable(varName, pszOldVal, buffSize);
  ...
  char *envVal = std::getenv(dstBuf.asArrayPtr());
  if (envVal == NULL)
  {
    return eKeyNotFound;
  }
