...
// To enable ODA RTTI:
// - uncomment ODRX_DECLARE_MEMBERS in .h file 
// - add rxInit, rxUninit calls to the code.
// - comment static createObject() methods in .h
//ODRX_NO_CONS_DEFINE_MEMBERS(OdBaseFileBuf, OdStreamBuf)
//ODRX_DEFINE_MEMBERS(OdRdFileBuf, OdBaseFileBuf, NEWOBJ_CONSTR, 0, 0, 0, OD_T("OdRdFileBuf"), OdString::kEmpty, OdString::kEmpty)
//ODRX_DEFINE_MEMBERS(OdWrFileBuf, OdBaseFileBuf, NEWOBJ_CONSTR, 0, 0, 0, OD_T("OdWrFileBuf"), OdString::kEmpty, OdString::kEmpty)
...
{ 
  ...
  {
    ODA_ASSERT_ONCE(!L"Exception in ~OdWrFileBuf");
  }
}
                         ...
                         Oda::FileCreationDisposition creationDisposition)
{
  ...
  const OdChar *pStr = filename.c_str();  // Get pStr as point to unicode buffer. 
  OdString nStr(pStr);               // Here is nStr with unicode buffer only.
  OdAnsiString nAnsiUtf8(nStr, CP_UTF_8); // Here is UTF-8 string as result conversion from unicode.
  const char* fName = nAnsiUtf8.c_str();
#else
...
OdRdFileBuf::OdRdFileBuf()
    : m_PhysFilePos(0), m_BufPos(0), 
      m_BytesLeft(0), m_BufBytes(0), m_pNextChar(nullptr), 
      m_pCurBuf(0), m_UsingBlock(-1), m_Counter(0L)
{
  init();
}
...
OdUInt32 OdBaseFileBuf::getAccessMode() const
{
  return (OdUInt32)m_accessMode;
}
